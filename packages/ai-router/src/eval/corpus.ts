...
//
// 30 anonymized alert scenarios + per-item rubric. The runner scores each
// output against:
//   - hard rubric checks (subject keywords, body must-contain / must-not-contain,
//     length window) → 60% of total
//   - LLM-as-judge soft score (tone, factual accuracy, action specificity)
//     → 40% of total
// Final score is the weighted average. ≥ 85 means "ship it"; <85 means
// "tune prompt or pick a stronger model".
//
// Future sessions extend this corpus per `INARI_LIVE_V0_3_HANDOFF.md`
// v0.3 S3 § Work — the eval harness pattern is the template every
// notify.* migration follows in S4.
