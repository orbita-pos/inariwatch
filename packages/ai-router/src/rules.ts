...
export const RULES: Record<TaskName, Rule> = {
  ...
  // 5.2 alert.* — hybrid. Together Mixtral-8x7B (platform-funded) when flag on,
  // OpenAI GPT-4o-mini otherwise. Actual runtime switching via
  // resolveTargetForPlatformFunded() in web/lib/ai/together-routing.ts.
  [TASKS.ALERT_AUTO_ANALYZE]: { primary: { substrate: "cloud", provider: "together" }, fallback: CLOUD_OPENAI, fallbackTriggers: ["cloud-error", "cloud-rate-limit"] },
  [TASKS.ALERT_CORRELATE]:    { primary: { substrate: "cloud", provider: "together" }, fallback: CLOUD_OPENAI, fallbackTriggers: ["cloud-error", "cloud-rate-limit"] },
  // Classifier uses fine-tuned model or heuristics — Together not applicable here.
  ...
  // 5.5 chat.* — Qwen3-32B (platform-funded Together) when flag on, OpenAI fallback.
  [TASKS.CHAT_CONVERSATIONAL]:   { primary: { substrate: "cloud", provider: "together" }, fallback: CLOUD_OPENAI, fallbackTriggers: ["cloud-error", "cloud-rate-limit"] },
  [TASKS.CHAT_CODE]:             { primary: { substrate: "cloud", provider: "together" }, fallback: CLOUD_OPENAI, fallbackTriggers: ["cloud-error", "cloud-rate-limit"] },
  [TASKS.CHAT_INTENT_CLASSIFY]:  { primary: CLOUD_GROQ, fallback: CLOUD_OPENAI, fallbackTriggers: ["cloud-error"] },
  // Inari Live pure-slash Phase 2 — single-shot natural-language → slash
  // translator. Together's Qwen3.5-9B is the primary on platform-funded
  // users (via the ASSIST bucket in `web/lib/ai/together-routing.ts`);
  // BYOK / non-routed cohorts fall back to OpenAI.
  [TASKS.CHAT_SUGGEST_COMMAND]:  { primary: { substrate: "cloud", provider: "together" }, fallback: CLOUD_OPENAI, fallbackTriggers: ["cloud-error", "cloud-rate-limit"] },
