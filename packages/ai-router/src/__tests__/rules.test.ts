...
describe("routing rules — S3 (post-flip)", () => {
  ...
  // Remaining notify.* tasks stay cloud until later sessions wire their surfaces.
  // After S4+S5 (Baileys), only digest/status-page/postmortem-prose remain cloud-only.
  it("notify.compose.{digest,status-page,postmortem-prose} remain cloud after S4+S5", () => {
    for (const task of [
