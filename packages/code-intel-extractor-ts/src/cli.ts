...
// stdout. The output is a single JSON document with all four record arrays
// plus diagnostics; consumers parse with `JSON.parse` and iterate.
//
// Flags:
//   --repo-path <abs>          (required)
//   --tsconfig <relative>      (optional, defaults to repo's tsconfig.json)
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (tok) {
      ...
      case "--pretty":
        ...
        break;
      default:
        // Ignore unknown flags rather than crashing — the indexer may
        // pass forwards-compatible ones.
        break;
    }
  }
  return args;
