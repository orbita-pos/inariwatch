...
/// Read the `origin` remote URL from a repo's `.git/config`. Used by
/// the `/github` slash command to resolve which GitHub project to
/// open. Returns `None` when:
///   - the repo isn't in `repos` (caller should treat as "no active repo"),
///   - `.git/config` doesn't exist (bare clone with submodules, etc.),
///   - the file has no `[remote "origin"]` block,
///   - the block has no `url =` line.
///
/// We deliberately do NOT validate that the URL is a github.com URL
/// here — the frontend `/github` handler does the validation so it
/// can show the right error message ("repo is on gitlab, not github").
#[tauri::command]
pub fn desktop_git_origin_url(
    repo_id: String,
    store: tauri::State<'_, Arc<Store>>,
    ...
    Ok(parse_git_origin_url(&content))
}

/// Parse a git config file body and return the `url` value of the
/// `[remote "origin"]` section, if present. Public so unit tests can
/// lock the parser against the four URL formats git accepts:
///
///   - `https://github.com/owner/name.git`
///   - `git@github.com:owner/name.git`
///   - `ssh://git@github.com/owner/name.git`
///   - bare `git://github.com/owner/name`
pub fn parse_git_origin_url(content: &str) -> Option<String> {
    ...
    for raw in content.lines() {
        ...
        if trimmed.starts_with('[') {
            // `[remote "origin"]` is the canonical form, but git also
            // accepts the slightly different `[remote.origin]` (rare)
            // and is whitespace-tolerant. We match exact + relaxed.
            in_origin = trimmed == r#"[remote "origin"]"#
                || trimmed == "[remote.origin]";
            continue;
        }
        if !in_origin {
            continue;
        }
        // Match `url = ...` with arbitrary whitespace around `=`.
        if let Some(rest) = trimmed.strip_prefix("url") {
            let rest = rest.trim_start();
            if let Some(after_eq) = rest.strip_prefix('=') {
                return Some(after_eq.trim().to_string());
            }
        }
    }
    None
...
#[cfg(test)]
mod parse_git_origin_url_tests {
    use super::parse_git_origin_url;
...
[core]
    ...
    filemode = false
[remote "origin"]
    url = https://github.com/orbita-pos/inariwatch.git
    fetch = +refs/heads/*:refs/remotes/origin/*
"#;
        assert_eq!(
            parse_git_origin_url(body).as_deref(),
            Some("https://github.com/orbita-pos/inariwatch.git"),
        );
    }
    ...
    fn parses_ssh_remote() {
        let body = r#"
[remote "origin"]
    url = git@github.com:orbita-pos/inariwatch.git
"#;
        assert_eq!(
            parse_git_origin_url(body).as_deref(),
            Some("git@github.com:orbita-pos/inariwatch.git"),
        );
    }

    #[test]
    fn returns_none_when_no_origin_remote() {
        // Real shape from a freshly-initialized repo with no remote.
        let body = r#"
[core]
    repositoryformatversion = 0
    filemode = false
"#;
        assert!(parse_git_origin_url(body).is_none());
    }
    ...
    fn ignores_non_origin_remotes() {
        let body = r#"
[remote "upstream"]
    url = https://github.com/upstream-fork/inariwatch.git
[remote "fork"]
    url = git@github.com:fork/inariwatch.git
"#;
        assert!(parse_git_origin_url(body).is_none());
    }

    #[test]
    fn picks_origin_when_multiple_remotes_exist() {
        let body = r#"
[remote "upstream"]
    url = https://github.com/upstream/inariwatch.git
[remote "origin"]
    url = https://github.com/owner/inariwatch.git
[remote "fork"]
    url = https://github.com/fork/inariwatch.git
"#;
        assert_eq!(
            parse_git_origin_url(body).as_deref(),
            Some("https://github.com/owner/inariwatch.git"),
        );
    }

    #[test]
    fn tolerates_no_whitespace_around_equals() {
        let body = r#"
[remote "origin"]
