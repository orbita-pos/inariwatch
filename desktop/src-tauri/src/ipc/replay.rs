...
pub async fn replay_against_patch_with_store(
    ...
    // Session 1 — bearer lives in OS keyring; SecretStore transparently
    // handles the settings-store fallback for pre-S1 installs.
    let dashboard_token = crate::cloud::keyring::SecretStore::new(store_arc.clone())
        .token()
        .filter(|s| !s.trim().is_empty());
