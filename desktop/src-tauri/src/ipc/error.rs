...
/// Bridge the AI-client error type into [`IpcError`]. Previously lived
/// in `ipc::chat` (the primary OpenAI consumer at the time); moved here
/// when the cloud chat path was removed in Phase 3 of the pure-slash
/// refactor (2026-05-15) so the remaining caller (`ipc::remediation`)
/// keeps compiling. The `NoKey` variant gets a concrete, actionable
/// message instead of the underlying display; everything else collapses
/// to `Internal` with the upstream `Display` text preserved.
