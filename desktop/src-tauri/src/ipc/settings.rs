...
/// Best-effort server-side revoke of THIS device's token followed by
/// an unconditional local clear. See `crate::cloud::auth::logout` for
/// the rationale (S1 R4: never strand the user even if the network is
/// down). Returns the post-clear `SafeSettings` snapshot so the
/// frontend's settings store can sync without a follow-up read.
#[tauri::command]
pub async fn desktop_logout(
    ...
    crate::cloud::auth::logout(&app, &store).await?;
    settings::safe_snapshot(&store).map_err(|e| e.to_string())
}
...
/// Local LLM chat inference toggle (Settings → AI → Local AI offline mode).
const SETTINGS_KEY_LOCAL_CHAT_ENABLED:  &str = "local_chat_enabled";
...
pub struct AiSettings {
    ...
    /// Local LLM offline mode (Settings → AI → Local AI). When true, chat
    /// routes to `local_ai_infer` instead of the cloud API. Default false.
...
pub fn get_ai_settings(
    ...
    Ok(AiSettings {
        ...
        local_chat_enabled:  read_bool_or(&state, SETTINGS_KEY_LOCAL_CHAT_ENABLED,  false)?,
    })
}
...
pub fn set_ai_settings(
    ...
    if let Some(b) = patch.local_chat_enabled {
        settings::set(&state, SETTINGS_KEY_LOCAL_CHAT_ENABLED, &b.to_string())?;
    }
