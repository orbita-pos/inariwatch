...
pub mod pool;
pub mod queries;
// Inari Live Phase 5.2 — recent-paths ring buffer powering the path
// picker. Capped at 20 entries; eviction by `last_used_at` ASC.
pub mod recent_paths;
// Inari Live Phase 5.5 completion — recent-contacts ring buffer
// populated by successful /whatsapp sends. Same shape as recent_paths
// but keyed by jid (E.164).
...
impl Store {
    ...
    /// Borrow the underlying r2d2 pool. Used by callers that need to
    /// hand a `SqlitePool` to a sub-component which manages its own
    /// connections (e.g. the chat-agent `AuditLog` in S11). Cloning
    /// the returned `SqlitePool` is cheap — r2d2 holds an Arc
    /// internally — so the typical pattern is `store.pool().clone()`.
