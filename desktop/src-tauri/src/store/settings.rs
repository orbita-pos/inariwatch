...
pub fn safe_snapshot(store: &Store) -> Result<SafeSettings> {
    // Session 1 — `dashboard_token` lives in the OS keyring (with the
    // SQL-settings-store fallback handled inside `SecretStore`). The
    // safe_snapshot only surfaces presence, so any non-empty value
    // counts.
    let dashboard_token_present = crate::cloud::keyring::SecretStore::new(
