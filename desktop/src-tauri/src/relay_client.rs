...
pub struct RelayConfig {
    ...
    /// Inari Live V1 — Session 3 wizard payload sink. When `Some`, the
    /// `project.wizard.open` task arm stores the incoming payload via
    /// `WizardStore::set_pending` so the React side can auto-render
    /// the Add-Project wizard. `None` in tests + early boot — the
    /// dispatcher then falls back to the generic stub.
    pub wizard_store: Option<Arc<crate::wizard::WizardStore>>,
    /// Tauri AppHandle threaded through so the wizard arm can emit
    /// `wizard:opened` directly. `None` on tests / pre-boot — the
    /// dispatcher logs and returns ack so the relay still gets a
        ...
        match msg {
            Ok(Message::Text(txt)) => {
                if let Ok(df) = serde_json::from_str::<DispatchFrame>(&txt) {
                    ...
                    let resp = handle_dispatch_full_with_wizard(
                        cfg.local_ai.as_ref(),
                        cfg.app_local_data_dir.as_ref(),
                        cfg.whatsapp.as_ref(),
                        cfg.wizard_store.as_ref(),
...
pub async fn handle_dispatch_full(
    ...
    handle_dispatch_full_with_wizard(local_ai, voice_dir, whatsapp, None, None, df).await
}

/// Inari Live V1 — Session 3 dispatcher variant with wizard hooks.
/// Wired into `connect_once` once `RelayConfig::wizard_store` /
/// `app_handle` are set on boot. The pre-S3 callsite stays valid
/// (passes None for the new args) so existing tests don't change.
pub async fn handle_dispatch_full_with_wizard(
    local_ai: Option<&Arc<LocalAI>>,
    voice_dir: Option<&Arc<std::path::PathBuf>>,
    whatsapp: Option<&Arc<SidecarManager>>,
    wizard_store: Option<&Arc<crate::wizard::WizardStore>>,
    ...
    match df.task.as_str() {
        ...
        "project.wizard.open" => handle_project_wizard_open(wizard_store, app_handle, df),
        _ => build_stub_response(df),
    }
}

/// Inari Live V1 — Session 3 wizard.open arm. Cheap: just stores the
/// payload + emits `wizard:opened`. The actual install pipeline runs
/// from the React side via the wizard IPC commands.
fn handle_project_wizard_open(
    wizard_store: Option<&Arc<crate::wizard::WizardStore>>,
    app_handle: Option<&tauri::AppHandle>,
    df: &DispatchFrame,
) -> ResponseFrame {
    let payload: crate::wizard::WizardPayload = match serde_json::from_value(df.payload.clone()) {
        Ok(p) => p,
        Err(e) => return error_response(df, format!("invalid wizard payload: {}", e)),
    };
    match (wizard_store, app_handle) {
        (Some(store), Some(app)) => {
            store.set_pending(app, payload.clone());
            ResponseFrame {
                ty: "response",
        ...
        _ => stub_with_note(df, "wizard_store not initialized — payload dropped"),
    }
}
