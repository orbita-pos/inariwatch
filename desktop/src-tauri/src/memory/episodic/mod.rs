...
fn kind_tag(ev: &DaemonEvent) -> Option<&'static str> {
    match ev {
        ...
        // S6 — chat-agent tool calls have their own dedicated audit
        // trail (`tool_invocations` table, written by the registry's
        // pipeline before the bus event is published). Persisting the
        // bus event too would double-count, so the episodic ledger
        // treats it the same as ChatTokenStream chatter.
        DaemonEvent::ChatToolCall { .. }          => None,
