...
async function handleLoginStart(
  ...
  process.stderr.write(
    `[whatsapp-sidecar] login_start received account_id=${params.account_id} label=${params.label}\n`,
  );
  ...
  // Persist the human-readable label so the Rust side can restore it
  // on the next app boot (resume_persisted_accounts reads this back).
  // Best-effort — a write failure here doesn't break the pair flow,
  // we'll just default to "Personal" on resume.
  await fs.mkdir(accountDir, { recursive: true });
  await fs
    .writeFile(join(accountDir, "label.txt"), params.label, { encoding: "utf-8" })
    .catch((writeErr) => {
      process.stderr.write(
        `[whatsapp-sidecar] label.txt write failed (non-fatal): ${String(writeErr)}\n`,
      );
    });
