...
export type WhatsAppEvent =
  ...
  // v0.3 S8 — inbound DM. Mirrors the Rust
  // `WhatsAppEvent::MessageReceived` variant. The session emits one of
  // these for every `messages.upsert` notification matching:
  //   - `notification` is `"notify"` (real new message; ignore "append"
  //     replays)
  //   - `key.remoteJid` ends in `@s.whatsapp.net` (DM, not group)
  //   - `message` carries `conversation` or `extendedTextMessage.text`
  //     (text DM only — image/audio/video filtered)
  //
  // The Rust bridge in `crate::messenger::whatsapp::sidecar_bridge`
  // double-checks `from_me` and `is_group` to defend against future
  // sidecar regressions.
  | {
