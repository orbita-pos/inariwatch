...
#[test]
fn fresh_db_applies_three_migrations_then_is_idempotent() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let db_path = tmp.path().join("inari-live").join("store.db");

    // First open: applies all 3.
    ...
    assert_eq!(count, 3, "expected 3 migrations applied on a fresh DB");
    drop(conn);
    drop(store);
    ...
    let conn = store.conn().expect("conn");
    let count = migrations::applied_count(&conn).expect("count");
    assert_eq!(count, 3, "second open must not duplicate migrations");
}
