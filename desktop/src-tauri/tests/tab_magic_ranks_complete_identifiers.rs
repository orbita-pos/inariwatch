//! Sesión 24 — n=3 sampling + ranking. The mock returns three
//! distinct candidates over three sequential POST requests:
//!
//!   1. `addNumb`         — ends mid-word (penalised)
//!   2. `add(a, b) {`     — ends on `{` (boundary-clean)
//!   3. `addNumber`       — ends on identifier boundary
//!
//! The handler should pick #2 or #3 (both score `+10` for boundary;
//! syntactic validity tilts toward #2). Either way, the candidate
//! ending mid-word (`addNumb`) MUST NOT win.
...
async fn completion_handler(
    ...
    // Three distinct candidates per request index (modulo 3).
    let body = match n % 3 {
        0 => r#"{"content":"addNumb","stop":false}"#,            // mid-word
        1 => r#"{"content":"add(a, b) { a + b }","stop":false}"#, // boundary-clean
        _ => r#"{"content":"addNumber","stop":false}"#,           // boundary-clean
    };
    let chunks: Vec<Result<Event, std::convert::Infallible>> = vec![
        Ok(Event::default().data(body.to_string())),
        Ok(Event::default().data(r#"{"content":"","stop":true,"stop_type":"stop"}"#.to_string())),
    ];
...
#[tokio::test]
async fn ranker_picks_boundary_clean_over_mid_word() {
    ...
    // Cursor right after `function ` — natural place to suggest a fn name.
    let text = "function ";
    let did_open = json!({
    ...
    let comp = json!({
        ...
        "params": {
            ...
            "position":     { "line": 0, "character": text.len() as u64 }
        }
    });
    ...
    assert_eq!(items.len(), 1, "expected exactly one ranked item");
    let insert: &str = items[0]["insertText"].as_str().expect("insertText is string");
    assert_ne!(insert, "addNumb",
        "ranker MUST NOT pick the mid-word candidate; got {insert}");
    // Either boundary-clean candidate is acceptable.
    assert!(
        insert == "add(a, b) { a + b }" || insert == "addNumber",
        "unexpected pick: {insert}",
    );
    assert_eq!(counter.load(Ordering::Relaxed), 3,
        "n=3 sampling should fire exactly 3 requests");
