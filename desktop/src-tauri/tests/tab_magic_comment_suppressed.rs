//! Sesión 24 — smart triggers suppress completion inside line and
//! block comments. Same shape as the string-literal test: empty list +
//! zero hits on the mock.
...
#[tokio::test]
async fn cursor_inside_rust_line_comment_returns_empty() {
    ...
    assert!(items.is_empty(), "expected empty completion in comment, got {items:?}");
