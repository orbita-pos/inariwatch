# Inari Live — bash shell hook (Sensor 2, Session 9)
# Sourced from ~/.bashrc when "Watch my terminal" is enabled.
# Requires bash-preexec (https://github.com/rcaloras/bash-preexec) to
# be sourced BEFORE this file — bash has no native preexec/precmd
# arrays. See ~/.inari/shell/README.md for installation steps; the
# Inari Live installer does NOT bundle bash-preexec.
# Forwards {cmd, cwd, exit_code, duration_ms, timestamp} to the
# daemon over a Unix domain socket. Privacy-scrubbing happens HERE,
# before any data leaves the shell.
...
# Hook into bash-preexec's arrays. If they aren't defined the user
# hasn't installed bash-preexec yet — silently no-op so sourcing this
# file never breaks an interactive shell.
if declare -p preexec_functions >/dev/null 2>&1; then
    if [[ ! " ${preexec_functions[*]} " == *" __inari_preexec_hook "* ]]; then
        preexec_functions+=(__inari_preexec_hook)
    fi
fi
if declare -p precmd_functions >/dev/null 2>&1; then
    if [[ ! " ${precmd_functions[*]} " == *" __inari_precmd_hook "* ]]; then
