...
export function DockShell() {
  ...
  useEffect(() => {
    // Cancelled-flag pattern. The three install* functions are async,
    // so a synchronous cleanup that races the install will see
    // `unlistenX = null` and never tear the listener down. Under
    // React.StrictMode in dev that produces two parallel listeners on
    // the second mount — each daemon:event then routes through
    // appendToken twice, duplicating the assistant bubble's content.
    // The flag flips on cleanup; when the promise resolves later, we
    // call unlisten immediately instead of stashing it.
    let cancelled = false;
    ...
    // S7 — ambient surfaces emit `chat:prefill` to stuff the input
    // box with a prompt (tray Quick Actions, OS notification action
    // callbacks). The listener flips the dock into conversation mode
    // so the input is visible.
