...
const VARIANT_CLASSES: Record<Variant, string> = {
  // 2026-05-07 chat-first reframe: accent is now warm cream (#EFE9DC).
  // White text on cream is invisible — primary uses `--accent-ink`
  // (#16161a) instead. Same dark text on hover (warmer cream).
  primary: cn(
  ...
  secondary: cn(
    "bg-[var(--card)] text-[var(--text)] border border-[var(--border-strong)]",
    "hover:bg-[var(--card-elevated)] hover:border-[var(--border-3)]",
    "active:bg-[var(--card-elevated)]",
  ),
  ghost: cn(
    "bg-transparent text-[var(--text-muted)] border border-transparent",
    "hover:bg-white/[0.025] hover:text-[var(--text)]",
