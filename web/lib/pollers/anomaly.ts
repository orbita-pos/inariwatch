...
/** Best-effort ML anomaly score + ADWIN drift detection (500 ms timeout).
 *  Pass workspaceId to also train the per-workspace SNARIMAX forecasting model.
 *  Returns null when INARI_ML_URL is unset or sidecar is unreachable. Never throws. */
async function mlAnomalyScore(
  metric: string,
  value: number,
  workspaceId?: string,
  ...
  if (!url) return null;
  try {
    const res = await fetch(`${url}/ml/anomaly-score`, {
      method: "POST",
    ...
    return res.json() as Promise<MlAnomalyResult>;
  } catch {
    return null;
  }
}

/** Best-effort SNARIMAX forecast (1 s timeout).
 *  Returns null when sidecar is unreachable or has insufficient data. Never throws. */
async function mlForecast(
  workspaceId: string,
  metric: string,
  horizonMinutes = 30,
  ...
  if (!url) return null;
  try {
    const res = await fetch(`${url}/ml/forecast`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ workspace_id: workspaceId, metric, horizon_minutes: horizonMinutes }),
      signal: AbortSignal.timeout(1000),
    });
    if (!res.ok) return null;
    const data = await res.json() as MlForecastResult;
    return data.insufficient_data ? null : data;
  } catch {
    return null;
  }
}
...
async function detectAlertSpikes(projectIds: string[]): Promise<AnomalyResult[]> {
  ...
  for (const projectId of projectIds) {
    ...
    // Try ML anomaly scoring first. The HalfSpaceTrees model learns the
    // distribution of `alert_rate_ratio` values over time, so it adapts to
    // projects with naturally bursty traffic instead of using static thresholds.
    // Also trains the per-project SNARIMAX model for forecasting.
    // Falls back to ratio thresholds when sidecar is unavailable.
    const ml = await mlAnomalyScore("alert_rate_ratio", ratio, projectId);

    let severity: "critical" | "warning" | null = null;
    if (ml !== null) {
      // ML path: trust the model; score > 0.7 → critical, otherwise warning.
      if (ml.is_anomaly) severity = ml.score > 0.7 ? "critical" : "warning";
    } else {
      // Fallback thresholds (ML unavailable).
    ...
    // Drift detection — ADWIN fires once when the baseline structurally shifts.
    // Uses "Drift:" prefix so auto-resolve skips it (requires manual ack).
    if (ml?.drift_detected) {
      const mag = ml.drift_magnitude != null ? ` (shift: ~${ml.drift_magnitude.toFixed(2)}×)` : "";
      results.push({
        projectId,
        severity: "warning",
        title: `Drift: alert rate baseline shifted`,
        body: `The normal error rate for this project has shifted significantly${mag}. This typically happens after a deploy, traffic change, or configuration update.\n\nThe anomaly detection model will adapt to the new baseline automatically. Review recent changes if this shift is unexpected.`,
        sourceIntegrations: [],
        isRead: false,
        isResolved: false,
      });
...
// ── 5. SNARIMAX forecasting — proactive alerts ────────────────────────────────

async function detectForecasts(projectIds: string[]): Promise<AnomalyResult[]> {
  const results: AnomalyResult[] = [];

  await Promise.all(
    projectIds.map(async (projectId) => {
      const fc = await mlForecast(projectId, "alert_rate_ratio", 30);
      if (!fc) return;

      // Need meaningful confidence and a predicted spike before alerting
      if (fc.confidence < 0.4) return;

      let severity: "critical" | "warning" | null = null;
      if (fc.predicted_value >= 5) severity = "critical";
      else if (fc.predicted_value >= 3) severity = "warning";
      if (!severity) return;

      const pct = Math.round(fc.confidence * 100);
      const mult = fc.predicted_value.toFixed(1);
      results.push({
        projectId,
        severity,
        title: `Anomaly: error rate spike predicted`,
        body: `ML forecasting predicts a ~${mult}× surge in the next 30 minutes (confidence: ${pct}%). Based on learned time-series patterns for this project.\n\nConsider reviewing recent deployments and infrastructure status before alerts begin firing.`,
        sourceIntegrations: [],
        isRead: false,
        isResolved: false,
      });
...
async function autoResolveStaleAnomalies(
  ...
  // "Drift:" prefix alerts are intentionally excluded — they require manual ack
  // since ADWIN fires once and the baseline shift is a permanent state change.
...
export async function detectAnomalies(projectIds: string[]): Promise<AnomalyResult[]> {
  ...
  const [spikes, repeating, integErrors, silent, forecasts] = await Promise.all([
    ...
    detectForecasts(projectIds).catch(() => [] as AnomalyResult[]),
  ]);

  return [...spikes, ...repeating, ...integErrors, ...silent, ...forecasts];
}
