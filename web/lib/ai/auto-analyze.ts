...
import { getProjectOwnerAIKey, getPlatformTogetherKey, PLATFORM_MODEL } from "./get-key";
import { resolveTargetForPlatformFunded, TOGETHER_ALERT_TIER } from "./together-routing";
import { correlateProjectAlerts } from "./correlate";
import { computeErrorFingerprint, extractEntitiesFromStackTrace } from "./fingerprint";
...
export async function autoAnalyzeAlert(alert: Alert): Promise<void> {
  ...
  //
  // Parallelized with the GLiNER entity extraction below — both are
  // independent I/O. With the ML sidecar's 2s timeout on
  // `extractEntitiesFromStackTrace`, the previous serial flow added that
  // 2s inline to every alert's prompt-build path. Promise.all overlaps
  // them so wall-clock is `max(DB query, sidecar call)` not the sum.
  const [fullTraceContext, mlEntities] = await Promise.all([
    getFullTraceContextForAlert(alert.id).catch(() => null),
    extractEntitiesFromStackTrace(alert.body ?? ""),
  ]);
  const entitySection = (() => {
    if (!mlEntities.length) return "";
    const files = mlEntities.filter(e => e.label === "filename").map(e => e.text).slice(0, 3);
    const fns = mlEntities.filter(e => e.label === "function_name").map(e => e.text).slice(0, 3);
    const parts: string[] = [];
    if (files.length) parts.push(`Files: ${files.join(", ")}`);
    if (fns.length) parts.push(`Functions: ${fns.join(", ")}`);
    return parts.length ? `\n\nStack trace entities: ${parts.join(". ")}.` : "";
  })();
  ...
  // Inari Live V1 Session 5 — post the diagnosis as the first assistant
  // message in the conversation thread. The conversation row itself was
  // created at alert-insert time (`createAlertIfNew`), so this only
  // appends. Idempotent: re-running auto-analyze on the same alert
  // would post a *new* assistant turn, which is the right behaviour for
  // "Try again" retries from the empty-state UI.
  try {
    const { conversations: convTable } = await import("@/lib/db");
    const { postAssistantMessage } = await import("@/lib/services/conversations.service");
    const [conv] = await db
      .select({ id: convTable.id })
      .from(convTable)
      .where(eq(convTable.anchorAlertId, alert.id))
      .limit(1);
    if (conv) {
      await postAssistantMessage(conv.id, {
        text: reasoning,
        meta: cascadeInfo
          ? {
              source: "auto-analyze",
    ...
    console.error("[auto-analyze] postAssistantMessage failed:", err instanceof Error ? err.message : err);
  }
...
function buildCascadeTiers(
  ...
  // Platform key cascade: Together → Groq → OpenAI
  const tiers: CascadeTier[] = [];

  // Tier 0: Together Mixtral-8x7B — when INARI_TOGETHER_PLATFORM_FUNDED_ENABLED=true
  const togetherTarget = resolveTargetForPlatformFunded("alert.auto-analyze");
  const togetherKey = togetherTarget ? getPlatformTogetherKey() : null;
  if (togetherTarget && togetherKey) {
    tiers.push({
      key: togetherKey.key,
  ...
  // Tier 1: Groq (ultra-cheap) — for auto_fix and triage_only (skip if Together active)
  if (!togetherTarget && GROQ_KEY && triageClass && triageClass !== "full_remediation") {
    tiers.push({
      key: GROQ_KEY,
  ...
  // Tier 2: GPT-4o-mini (standard) — always present as fallback
  tiers.push({
    key: userKey,
