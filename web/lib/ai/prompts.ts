...
export type RemediationContext = {
  ...
  /** P1.1 — Recent anomaly/drift alerts from the ML poller (last 24h).
   *  Injected so the AI knows whether the project is in an abnormal
   *  performance or traffic state that could explain the error. */
  anomalyContext: string | null;
  /** P1.4 — Alerts that fired within ±10 min of this one for the same
   *  project — likely candidates for cascade failure or a shared root cause. */
  relatedAlertsContext: string | null;
  /** P1.3 — How many times this exact fingerprint fired in the last 24h.
   *  Gives the AI a blast-radius estimate: "only me" vs "500 users". */
...
export type MemoryHint = {
  ...
  filesFixed: string[];
  confidence: number;
  /** PR URL for successful past fixes — lets the AI see exactly what was merged. */
  prUrl?: string | null;
};
...
export function buildDiagnosePrompt(
  ...
  if (context?.impactContext) contextSections.push(`BLAST RADIUS (how many times this exact error fingerprint fired in the last 24h — higher count = more users affected):\n${context.impactContext}`);
  if (context?.anomalyContext) contextSections.push(`ANOMALY / DRIFT SIGNALS (recent ML-detected anomalies for this project — may explain the error if the system is under abnormal load or traffic):\n${context.anomalyContext.slice(0, 800)}`);
  if (context?.relatedAlertsContext) contextSections.push(`RELATED ALERTS (other errors from this same project in the last 10 minutes — may share a root cause or be part of a cascade):\n${context.relatedAlertsContext.slice(0, 1200)}`);
...
export function buildFixPrompt(
  ...
  stackContext?: string,
  astContexts?: Record<string, { enclosingFunction: string | null; enclosingClass: string | null; enclosingTryCatch: boolean; callExpression: string | null; errorLine: number }>,
  pastHints?: MemoryHint[],
): string {
  // Build file contents with import analysis and optional AST context
  const fileContents = files
    .map((f) => {
      const imports = extractImports(f.content);
      const importSection = imports ? `\nIMPORTS IN THIS FILE (use ONLY these libraries):\n${imports}\n\n` : "";

      const ast = astContexts?.[f.path];
      let astSection = "";
      if (ast) {
        const parts: string[] = [];
        if (ast.enclosingFunction) parts.push(`Enclosing function: ${ast.enclosingFunction}`);
        if (ast.enclosingClass) parts.push(`Enclosing class: ${ast.enclosingClass}`);
        if (ast.enclosingTryCatch) parts.push(`Inside try/catch: yes`);
        if (ast.callExpression) parts.push(`Call expression: ${ast.callExpression}`);
        if (parts.length > 0) {
          astSection = `\nAST context at error location (line ${ast.errorLine}):\n${parts.slice(0, 4).map((p) => `- ${p}`).join("\n")}\n\n`;
        }
      }

      return `--- ${f.path} ---${importSection}${astSection}${f.content.slice(0, 10000)}`;
    })
    .join("\n\n");
...
Analyze the CI error carefully to understand why the previous fix failed.`;
  ...
  const pastHintsSection = pastHints && pastHints.length > 0
    ? `\n\nPAST FIX ATTEMPTS FOR THIS EXACT ERROR (use the most successful approach as your baseline):\n${
        pastHints
          .map((h, i) => [
            `Attempt ${i + 1} [confidence ${h.confidence}%]: ${h.rootCause}`,
            `  Fix: ${h.fixSummary}`,
            `  Files: ${h.filesFixed.join(", ")}`,
            h.prUrl ? `  Merged PR: ${h.prUrl}` : null,
          ].filter(Boolean).join("\n"))
          .join("\n---\n")
      }`
    : "";
...
${retryContext}${pastHintsSection}${codebaseSection}${antiPatternContext ?? ""}${stackSection}
...
// ── Inari Guard — user-driven `/test <path>` flow ──────────────────────────
//
// Three-pass model strategy (distinct from the regression-test prompts
// below, which fire automatically after a remediation):
//
//   Pass 1  SYSTEM_TEST_PLANNER  — Qwen3-Coder-Next-FP8 ($0.50/$1.20)
//   Pass 2  SYSTEM_TEST_WRITER   — Qwen3-Coder-480B-A35B ($2.00 flat)
//   Pass 3  SYSTEM_TEST_REVIEWER — Qwen3-235B-A22B (thinking, $0.20/$0.60)
//
// Total cost per file: ~$0.10-0.15. The reviewer can reject the writer's
// output and force a re-write with feedback (max 2 iterations).

export const SYSTEM_TEST_PLANNER = `You are a senior test engineer planning what to test.

Given a source file plus its callers and existing test conventions, produce a JSON test plan with one entry per case to write.

Coverage philosophy: prefer FEW deep tests over MANY shallow ones. Skip getters, setters, type guards, and pure passthroughs. Focus on:
1. The HAPPY PATH each exported function/method actually serves
2. EDGE CASES that are realistic in this codebase (null where the type says optional, empty arrays, boundary values evident from the code)
3. ERROR PATHS the function explicitly handles (thrown, rejected, status returns)
4. STATE TRANSITIONS for stateful objects (before/after each public mutator)

For EACH case provide:
- name:        camelCase identifier matching the existing test style
- target:      "fileName.functionName" or "fileName.ClassName#method"
- scenario:    one-sentence "given X, when Y, then Z"
- inputs:      minimal example values (literal types, not just type names)
- expected:    what the assertion should check (NOT how to assert it)
- mocks:       array of imports that must be mocked, with brief reason. Empty array if none.
- priority:    "high" (critical-path) | "medium" | "low" (nice-to-have, may be skipped)

Output ONLY a valid JSON object: { "framework": "vitest|jest|playwright|mocha|bun-test|deno-test", "cases": [...] }
Do NOT include test code in the plan — only the spec for what each test should verify.`;

export const SYSTEM_TEST_WRITER = `You are a senior test engineer writing test code.

You receive: a test plan (JSON), the source file, and 1-3 EXISTING test files from the same repo for style matching.

Your output is the COMPLETE content of a single new test file that:
1. Matches the EXACT style of the existing tests (imports, describe/test choice, assertion library, async patterns, mock helpers).
2. Implements every case marked priority=high in the plan, plus priority=medium when it fits naturally.
3. Skips priority=low cases unless they require less than 5 lines.
4. Has at least ONE meaningful expect()/assert per it()/test() — NEVER use expect(true).toBe(true) or similar always-passing assertions.
5. Avoids hardcoded timeouts (waitForTimeout, sleep, setTimeout for waits). Use the existing patterns (waitFor, polling, event-based) the test file uses.
6. Mocks ONLY what the plan flags. Do not invent extra mocks.
7. Names test cases human-readable: "returns user when token valid", not "test1" or "validateToken_HappyPath".
8. Keeps file size under 500 lines. If the plan is large, prioritize and TODO-comment the rest.

If you cannot implement a planned case (e.g., the source code doesn't actually have that behavior), drop it silently — do NOT generate a placeholder that always passes.

Output ONLY a JSON object: { "path": "tests/fileName.test.ts", "content": "...the complete file..." }`;

export const SYSTEM_TEST_REVIEWER = `You are a strict code reviewer of AI-generated tests.

You receive: the test plan and the generated test file. Decide if the tests are MERGE-READY.

REJECT (set approved=false) if ANY of the following:
- Any it()/test() has no expect()/assert call.
- An expect() is trivially true: expect(true).toBe(true), expect(1).toBe(1), expect(x).toBeDefined() when x is a literal.
- Hardcoded waits (waitForTimeout, setTimeout with numeric delays for synchronization).
- Tests that import the source but never call its exports.
- Imports that don't exist in the source file or in declared dependencies.
- Mocks created but never used.
- Magic strings/numbers that won't fail meaningfully (e.g., "test" or 42 with no relation to scenario).
- Test names that describe IMPLEMENTATION (e.g., "calls fetchUser internally") instead of BEHAVIOR.

APPROVE (set approved=true) only if all of the above are clean.

Output ONLY JSON:
{
  "approved": boolean,
  "score": 0-100,
  "rejected_cases": [{ "name": "case-name", "reason": "why" }],
  "concerns": ["..."],
  "rewrite_hint": "If approved=false, one paragraph telling the writer what to change. Otherwise empty string."
}`;
...
export interface TestPlanCase {
  ...
  inputs: unknown;
  expected: string;
  mocks: { import: string; reason: string }[];
  priority: "high" | "medium" | "low";
}

export interface TestPlan {
  framework: "vitest" | "jest" | "playwright" | "mocha" | "bun-test" | "deno-test";
  cases: TestPlanCase[];
}
...
export interface TestReviewResult {
  approved: boolean;
  score: number;
  rejected_cases: { name: string; reason: string }[];
  concerns: string[];
  rewrite_hint: string;
}
...
export function buildTestPlanPrompt(args: {
  ...
  const { filePath, fileContent, callers = [], existingTests = [], frameworkHint } = args;

  const callerSection = callers.length === 0
    ? "(no callers found in the repo — file may be entry-point or new)"
    : callers
        .slice(0, 5)
        .map((c) => `--- ${c.path} ---\n${c.snippet.slice(0, 800)}`)
        .join("\n\n");

  const styleSection = existingTests.length === 0
    ? "(no existing tests — choose the most common framework for this language ecosystem)"
    : existingTests
        .slice(0, 3)
        .map((t) => `--- ${t.path} ---\n${t.content.slice(0, 1500)}`)
        .join("\n\n");

  const hint = frameworkHint ? `\nFRAMEWORK HINT FROM USER: ${frameworkHint}\n` : "";

  return `Plan tests for this file.

SOURCE FILE (${filePath}):
${fileContent.slice(0, 8000)}

CALLERS (how this file is used elsewhere in the repo — use to identify the real happy path):
${callerSection}
...
export function buildTestWritePrompt(args: {
  filePath: string;
  fileContent: string;
  plan: TestPlan;
  existingTests: { path: string; content: string }[];
  ...
  const { filePath, fileContent, plan, existingTests, rewriteHint } = args;

  const styleSection = existingTests.length === 0
    ? "(no style examples — use idiomatic " + plan.framework + ")"
    : existingTests
        .slice(0, 3)
        .map((t) => `--- ${t.path} ---\n${t.content.slice(0, 2000)}`)
        .join("\n\n");

  const rewriteSection = rewriteHint
    ? `\nREVIEWER FEEDBACK (your previous attempt was rejected — fix these issues):\n${rewriteHint}\n`
    : "";

  return `Write the test file from this plan.
...
STYLE EXAMPLES (match these):
${styleSection}
${rewriteSection}
Return the JSON with the complete test file now.`;
}

export function buildTestReviewPrompt(args: {
  plan: TestPlan;
  generatedTests: { path: string; content: string };
}): string {
  const { plan, generatedTests } = args;
  return `Review these generated tests.

PLAN THE WRITER WAS GIVEN:
${JSON.stringify(plan, null, 2)}

GENERATED TEST FILE (${generatedTests.path}):
${generatedTests.content.slice(0, 12000)}

Return the JSON verdict now.`;
}

// ── Regression test generator (post-fix flow — separate from /test) ─────────
