...
describe("buildDiagnosePrompt", () => {
  ...
  // ── P1 context upgrades (commit 5ea9aab2) — positive assertions ─────
  it("includes ANOMALY section when anomalyContext is populated", () => {
    const context: RemediationContext = {
      ...
      userContext: null,
      requestContext: null,
      anomalyContext: "Anomaly: error rate spike (score 0.82) at 14:23\nDrift: baseline shift +2.4σ",
      relatedAlertsContext: null,
      impactContext: null,
    };
    const prompt = buildDiagnosePrompt(alert, repoFiles, context);
    ...
    expect(prompt).toContain("baseline shift");
  });

  it("includes RELATED ALERTS section when relatedAlertsContext is populated", () => {
    const context: RemediationContext = {
      sentryStackTrace: null,
      sentryIssueDetails: null,
      vercelBuildLogs: null,
      ...
      requestContext: null,
      anomalyContext: null,
      relatedAlertsContext: "- [error] TypeError in src/auth.ts\n- [warn] Slow query in users.findById",
      impactContext: null,
    };
    const prompt = buildDiagnosePrompt(alert, repoFiles, context);
    expect(prompt).toContain("RELATED ALERTS");
    expect(prompt).toContain("TypeError in src/auth.ts");
    expect(prompt).toContain("Slow query");
  });

  it("includes BLAST RADIUS section when impactContext is populated", () => {
    const context: RemediationContext = {
      sentryStackTrace: null,
      sentryIssueDetails: null,
      vercelBuildLogs: null,
      ...
      anomalyContext: null,
      relatedAlertsContext: null,
      impactContext: "47 occurrences",
    };
    const prompt = buildDiagnosePrompt(alert, repoFiles, context);
    expect(prompt).toContain("BLAST RADIUS");
    expect(prompt).toContain("47 occurrences");
  });
});
...
describe("buildFixPrompt", () => {
  ...
  // ── P0 context upgrades (commit 1a37f88d) — positive assertions ─────
  it("includes PAST FIX ATTEMPTS section when pastHints is populated", () => {
    const pastHints: MemoryHint[] = [{
      alertTitle: "Same TypeError",
      rootCause: "Stale session token",
      fixSummary: "Added refresh-token retry middleware",
      filesFixed: ["web/middleware.ts"],
      confidence: 88,
    }];
    ...
    expect(prompt).toContain("PAST FIX ATTEMPTS FOR THIS EXACT ERROR");
    expect(prompt).toContain("Stale session token");
    expect(prompt).toContain("Added refresh-token retry middleware");
    expect(prompt).toContain("web/middleware.ts");
    expect(prompt).toContain("88%");
  });

  it("includes prUrl in past hints when present (P0.1)", () => {
    const pastHints: MemoryHint[] = [{
      alertTitle: "Same TypeError",
      rootCause: "Stale session token",
      fixSummary: "Added refresh-token retry middleware",
      filesFixed: ["web/middleware.ts"],
      confidence: 88,
      prUrl: "https://github.com/orbita-pos/inariwatch/pull/4242",
    }];
    const prompt = buildFixPrompt(
      diagnosis,
      files,
    ...
    expect(prompt).toContain("Merged PR:");
    expect(prompt).toContain("https://github.com/orbita-pos/inariwatch/pull/4242");
  });

  it("omits Merged PR line when prUrl is missing", () => {
    const pastHints: MemoryHint[] = [{
      alertTitle: "Same TypeError",
      rootCause: "Stale session token",
      fixSummary: "Added refresh-token retry middleware",
    ...
    expect(prompt).not.toContain("Merged PR:");
  });
});
