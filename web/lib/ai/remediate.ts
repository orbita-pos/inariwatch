...
import { getTogetherOverride, TOGETHER_QWEN36_PLUS } from "./together-routing";
...
// `cleanJSON` lives in `./json-utils` (imported at top of this file).
// Originally a private helper here; promoted to a shared module so
// test-generator.ts and other AI flows can reuse the same heuristic.
...
export async function runRemediation(sessionId: string, emit: Emit): Promise<void> {
  ...
  try {
    ...
    // Platform-funded: override fix generation to Qwen3.6-Plus (78.8% SWE-bench,
    // beats GPT-5.4 at ~7x lower cost). BYOK users keep their own provider/model.
    const tFix = getTogetherOverride("code.fix", aiKey.isPlatformKey);
    const fixKey      = tFix?.key      ?? aiKey.key;
    const fixProvider = tFix?.provider ?? aiKey.provider;
    const fixModel    = tFix?.model    ?? remModel;
    ...
    // ── AST CONTEXT (non-blocking) ─────────────────────────────────────────────
    // Inject structural context (enclosing function, class, try/catch, call site)
    // so the AI knows WHERE in the code the error fired.
    type ASTCtxMap = Record<string, {
    ...
    let astContexts: ASTCtxMap = {};
    try {
      const { extractASTContext, inferLanguage, extractErrorLineForFile } = await import("@/lib/code-intelligence/ast-extractor");
      for (const file of fileContents) {
        const lang = inferLanguage(file.path);
        if (!lang) continue;
        const lineNum = extractErrorLineForFile(alert.body, file.path);
        if (!lineNum) continue;
        const ctx = await extractASTContext(file.content, lineNum, lang);
        if (ctx) astContexts[file.path] = ctx;
      }
    } catch { /* non-blocking — AST extraction must never fail remediation */ }
    ...
    while (attempt <= session.maxAttempts) {
      ...
      if (useContainerAgent) {
        ...
        try {
          // Use the resolved AI key (platform OpenAI/Together or user's BYOK key).
          // Platform-funded path overrides explore + fix to Together Qwen via
          // getTogetherOverride; BYOK + flag-off paths keep aiKey.provider's
          // defaults. This is the HOT PATH — the worker handles the majority
          // of platform-funded remediations, so the override has to be here
          // (the single-shot block below was the original migration point but
          // it only fires on retry / fallback).
          const { resolveModel } = await import("./models");
          const tContainerExplore = getTogetherOverride("explore", aiKey.isPlatformKey);
          const tContainerFix     = getTogetherOverride("code.fix.container", aiKey.isPlatformKey);
          const containerApiKey   = tContainerFix?.key      ?? aiKey.key;
          const containerProvider = tContainerFix?.provider ?? aiKey.provider;
          const containerExplore  = tContainerExplore?.model ?? resolveModel("analysis", aiKey.provider, aiKey.modelPrefs);
          const containerFix      = tContainerFix?.model    ?? resolveModel("remediation", aiKey.provider, aiKey.modelPrefs);
      ...
      if (useAgentic) {
        ...
        try {
          ...
          // Use the resolved AI key (platform OpenAI/Together or user's BYOK key).
          // Same override pattern as the container worker block above —
          // platform-funded sessions route to Together Qwen, BYOK + flag-off
          // keep aiKey.provider defaults.
          const { resolveModel } = await import("./models");
          const tAgenticExplore = getTogetherOverride("explore", aiKey.isPlatformKey);
          const tAgenticFix     = getTogetherOverride("code.fix.agent-loop", aiKey.isPlatformKey);
          const agenticApiKey   = tAgenticFix?.key      ?? aiKey.key;
          const agenticProvider = tAgenticFix?.provider ?? aiKey.provider;
          const agenticExplore  = tAgenticExplore?.model ?? resolveModel("analysis", aiKey.provider, aiKey.modelPrefs);
          const agenticFix      = tAgenticFix?.model    ?? resolveModel("remediation", aiKey.provider, aiKey.modelPrefs);
      ...
      if (!fix) {
        ...
        try {
          ...
          fixRaw = await callAIWithRetry(fixKey, systemWithOverlays, [
            { role: "user", content: buildFixPrompt(diagnosis.diagnosis, fileContents, alert.body, previousAttempt, remediationContext?.codebaseContext, antiPatternCtx, stackContext, Object.keys(astContexts).length > 0 ? astContexts : undefined, pastHints.length > 0 ? pastHints : undefined) },
          ], {
            maxTokens: 4096,
            timeout: 60000,
            model: fixModel,
      ...
      // Qwen3-32B for security scan + cheap self-review (platform-funded).
      // GPT-5.4 is NOT used here — these are analysis gates, not fix generation.
      const tScan = getTogetherOverride("code.review.security", aiKey.isPlatformKey);
      const scanModel = tScan?.model ?? resolveModel("analysis", aiKey.provider, aiKey.modelPrefs);
      const scanProvider = tScan?.provider ?? aiKey.provider;
      const scanKey = tScan?.key ?? aiKey.key;
      const callAIScan = (key: string, system: string, msgs: { role: "user"; content: string }[]) =>
        callAI(scanKey, system, msgs, {
          model: scanModel,
      ...
      const tReview = getTogetherOverride("code.review.self", aiKey.isPlatformKey);
      const cheapReviewModel = tReview?.model ?? resolveModel("analysis", aiKey.provider, aiKey.modelPrefs);
      ...
      const prePushDag = await runGateDag([
        ...
        },
        {
          ...
          dependsOn: [],
          run: () => callAI(tReview?.key ?? aiKey.key, cheapReviewerWithOverlays, reviewPromptMessages, {
            maxTokens: tReview ? 1024 : 512,
            // Qwen3-235B with thinking mode runs 2-4x slower than GPT-5.4
            // at equivalent token count (chat-template `<think>` blocks
            // before the final answer). Bump to 90s when thinking is on
            // so mid-stream timeouts don't fire false-positives.
            timeout: tReview ? 90000 : 30000,
            model: cheapReviewModel,
            provider: tReview?.provider ?? aiKey.provider,
            // Qwen3 thinking mode for deeper code review reasoning.
            // Silently ignored if tReview is null (non-Together provider)
            // or if the model isn't in the supported thinking family
            // (see ai-router/_openai-compat-shared.ts::modelSupportsThinking).
            thinkingBudget: tReview ? 2000 : undefined,
            log: reviewLogCtx,
          }),
        },
      ]);
