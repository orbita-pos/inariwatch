...
export async function correlateProjectAlerts(
  ...
  // Use Together Mixtral-8x7B when the platform flag is on (cheaper, confirmed serverless).
  const togetherTarget = aiKey.isPlatformKey
    ? resolveTargetForPlatformFunded("alert.correlate")
    : null;
  const togetherKey = togetherTarget ? getPlatformTogetherKey() : null;
  const effectiveKey = togetherKey ?? aiKey;
  ...
  try {
    summary = await callAI(
      ...
      [{ role: "user", content: prompt }],
      {
        ...
        model: togetherTarget ? TOGETHER_ALERT_TIER : (aiKey.isPlatformKey ? PLATFORM_MODEL : undefined),
        ...(proj ? {
          log: {
            userId: proj.userId,
            projectId,
