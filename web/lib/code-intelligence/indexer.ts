...
export async function indexRepository(opts: IndexOptions): Promise<{ repoId: string; chunksIndexed: number }> {
  ...
  const embeddingKey = opts.embeddingKey ?? openaiKey;
  ...
  try {
    ...
    // 7. Generate embeddings (Voyage Code 3 preferred, OpenAI fallback)
    if (embeddingKey) {
      emit({ phase: "embeddings", current: 0, total: chunks.length, message: "Generando embeddings (Voyage Code 3)..." });
