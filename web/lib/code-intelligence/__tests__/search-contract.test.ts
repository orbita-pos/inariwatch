...
describe("search.searchCode — v1 baseline contract", () => {
  ...
  it("vector+BM25 path when openaiKey is omitted (local nomic inference used)", async () => {
    executeMock
      .mockResolvedValueOnce({ rows: [chunkRow(0, "validate")] }) // vector via local model
      .mockResolvedValueOnce({ rows: [chunkRow(1, "verify")] });   // bm25
    ...
    // Both vector and BM25 run — local nomic replaces the OpenAI key requirement.
