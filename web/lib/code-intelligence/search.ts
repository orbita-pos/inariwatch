...
import { embedQuery, LOCAL_EMBED_KEY } from "./embeddings";
...
export async function searchCode(
  ...
  // Vector search always runs — uses local nomic inference when no cloud key is supplied.
  const [vectorResults, bm25Results] = await Promise.all([
    vectorSearch(query, repoId, openaiKey ?? LOCAL_EMBED_KEY, options?.fileFilter),
