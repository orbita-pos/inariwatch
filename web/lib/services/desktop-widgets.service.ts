...
// ── Workspace summary (cloud.get_workspace_summary) ──────────────────────────
//
// Aggregated dashboard one-liner the desktop agent calls when the user
// asks open-ended workspace-level questions like "how are things?". The
// shape is a strict superset of `StatusSummary` plus the two highlights
// the agent wants on hand (`topNoisyProject`, `onCallSummary`) so the
// LLM can answer follow-ups like "what's making the most noise?" or
// "who's on-call?" without a second round-trip.

export type WorkspaceSummary = {
  /** Total alerts created in the last 24h across all visible projects. */
  ...
  monitorsTotal: number;
  projectCount: number;
  /** Project with the most alerts in the last 24h (null if zero alerts). */
  topNoisyProject: { id: string; name: string; alerts24h: number } | null;
  /** Active on-call primary across visible projects (first match). */
  onCallSummary: {
    scheduleId: string;
    projectId: string;
    projectName: string;
  ...
  /** ISO timestamp of the most recent alert (any severity). */
  lastAlertAt: string | null;
};
...
export async function getWorkspaceSummary(
  ...
  const [statusSummary, perProject, oncallStatus] = await Promise.all([
    getStatusSummary(projectIds),
    // Per-project alert counts in the last 24h — used to pick the
    // top noisy project.
    db
      .select({
        projectId: alerts.projectId,
      ...
      .from(alerts)
      .where(
        and(inArray(alerts.projectId, projectIds), gt(alerts.createdAt, since)),
      )
      .groupBy(alerts.projectId),
    getOncallStatus(projectIds),
  ]);

  let topNoisyProject: WorkspaceSummary["topNoisyProject"] = null;
  if (perProject.length > 0) {
    const top = perProject.reduce((acc, row) =>
      row.count > acc.count ? row : acc,
    );
    if (top.count > 0) {
      const [name] = await db
        .select({ name: projects.name })
        .from(projects)
        .where(eq(projects.id, top.projectId))
        .limit(1);
      topNoisyProject = {
        id: top.projectId,
  ...
  const totalAlerts24h = perProject.reduce((acc, r) => acc + r.count, 0);

  const oncallFirst = oncallStatus.schedules.find((s) => s.primary !== null);
  const onCallSummary = oncallFirst
    ? {
        scheduleId: "",
        projectId: oncallFirst.projectId,
        projectName: oncallFirst.projectName,
        primary: oncallFirst.primary,
      }
    : null;

  return {
    totalAlerts24h,
    alertsCritical24h: statusSummary.alertsCritical24h,
    alertsWarning24h: statusSummary.alertsWarning24h,
    monitorsDown: statusSummary.monitorsDown,
    monitorsTotal: statusSummary.monitorsTotal,
    projectCount: projectIds.length,
    topNoisyProject,
    onCallSummary,
    lastAlertAt: statusSummary.lastAlertAt,
  };
}

// ── Project health (cloud.get_project_health) ────────────────────────────────

export type ProjectHealth = {
  projectId: string;
  projectName: string;
  /** "ok" | "warning" | "critical" — derived from the live signals. */
  state: "ok" | "warning" | "critical";
  /** Alerts created in the last 24h, by severity. */
  alerts24h: { total: number; critical: number; warning: number; info: number };
  uptime: {
    monitorsTotal: number;
    monitorsDown: number;
    /** Last response time across this project's monitors (avg of last checks). */
    avgResponseMs: number | null;
  };
  lastDeploy: {
    title: string;
  ...
  /** Active integration services for this project (`["vercel","github",...]`). */
  integrations: string[];
};
...
export async function getProjectHealth(
  ...
  const [severityCounts, monitorRows, lastChecks, lastDeployRow, integRows] =
    await Promise.all([
      db
        .select({
          severity: alerts.severity,
        ...
        .from(alerts)
        .where(and(eq(alerts.projectId, projectId), gt(alerts.createdAt, since)))
        .groupBy(alerts.severity),
      db
        .select({
          id: uptimeMonitors.id,
      ...
      // Last response time per monitor, joined later.
      db.execute(sql`
        SELECT DISTINCT ON (monitor_id) monitor_id, response_time_ms
        FROM uptime_checks
        WHERE monitor_id IN (
          SELECT id FROM uptime_monitors WHERE project_id = ${projectId} AND is_active = true
        )
        ORDER BY monitor_id, checked_at DESC
      `),
      db
        ...
        .limit(40),
      db
        .select({ service: projectIntegrations.service })
  ...
  const critical = severityCounts.find((c) => c.severity === "critical")?.count ?? 0;
  const warning = severityCounts.find((c) => c.severity === "warning")?.count ?? 0;
  const info = severityCounts.find((c) => c.severity === "info")?.count ?? 0;
  const total = critical + warning + info;

  const monitorsDown = monitorRows.filter((m) => m.consecutiveFailures > 0).length;
  const samples = (lastChecks.rows as { response_time_ms: number | null }[])
    .map((r) => r.response_time_ms)
    .filter((v): v is number => typeof v === "number");
  const avg =
    samples.length > 0
  ...
  // First deploy-shaped alert. Same DEPLOY_TITLE_RE heuristic the
  // dashboard `/api/desktop/deploys` uses — kept inline so this function
  // doesn't depend on internal helpers below.
  const deployTitleRe = /\b(deploy|deployment|build)\b/i;
  const lastDeployHit = lastDeployRow.find((a) => {
    const sources = (a.sourceIntegrations ?? []) as string[];
    return (
      sources.some((s) => s === "vercel" || s === "github") ||
      deployTitleRe.test(a.title)
    );
  });
  let lastDeploy: ProjectHealth["lastDeploy"] = null;
  if (lastDeployHit) {
    const t = lastDeployHit.title.toLowerCase();
    let state: "success" | "failed" | "building" | "unknown" = "unknown";
    if (t.includes("fail") || lastDeployHit.severity === "critical") state = "failed";
    else if (t.includes("succe") || lastDeployHit.isResolved) state = "success";
    else if (t.includes("build")) state = "building";
    lastDeploy = {
      title: lastDeployHit.title,
      state,
      createdAt: new Date(lastDeployHit.createdAt).toISOString(),
    };
  }

  let state: ProjectHealth["state"] = "ok";
  if (critical > 0 || monitorsDown > 0) state = "critical";
  ...
  return {
    ...
    state,
    alerts24h: { total, critical, warning, info },
    uptime: {
      monitorsTotal: monitorRows.length,
      monitorsDown,
      avgResponseMs: avg,
    },
    lastDeploy,
    integrations: integRows.map((r) => r.service).sort(),
  };
}
