...
export async function POST(req: Request) {
  ...
  try {
    ...
    return new NextResponse(
      renderHtml("Acknowledged", "The alert has been acknowledged and escalation has been stopped.", true),
      { status: 200, headers: { "Content-Type": "text/html" } }
    );
  } catch {
    return new NextResponse(
      renderHtml("Error", "Failed to acknowledge the alert.", false),
...
<head>
  ...
  <title>Acknowledge Alert - InariWatch</title>
  <style>
    ...
    .icon { display:flex;align-items:center;justify-content:center;width:48px;height:48px;border-radius:50%;background-color:#eab30820;color:#eab308;font-size:24px;font-weight:bold;margin:0 auto 16px; }
