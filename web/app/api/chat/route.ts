...
export async function POST(req: NextRequest) {
  ...
  // Per-user rate limit: burst (5/min platform, 10/min BYOK) + daily cap (50/day platform)
  const rl = await checkChatRateLimit(userId, aiKey.isPlatformKey ?? false);
  if (!rl.ok) {
    const msg =
      rl.reason === "burst"
        ? "You're sending messages too fast. Wait a moment before continuing."
        : "You've reached your daily chat limit (50 messages). Limits reset at midnight UTC. Add your own API key in Settings for unlimited access.";
    return Response.json({ role: "assistant", content: msg });
  }
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        const chatModel = t?.model ?? resolveModel("chat", aiKey.provider, aiKey.modelPrefs);
        const it = dispatchStream({
          mode: "stream",
          task: TASKS.CHAT_CONVERSATIONAL,
          apiKey: t?.key ?? aiKey.key,
          systemPrompt: SYSTEM_OPS,
          messages: aiMessages,
          maxTokens: 1024,
          model: chatModel,
          provider: t?.provider,
          // Qwen3-235B-A22B p95 latency is ~2x GPT-5.4 at equivalent token
          // count even without thinking mode. Bump from 60s → 90s when
          // routing through Together so streaming doesn't time out mid-
          // generation on the longer tail.
          timeout: t ? 90_000 : 60_000,
