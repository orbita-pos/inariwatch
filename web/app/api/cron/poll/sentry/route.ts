...
export async function GET(req: Request) {
  ...
  const integrations = integrationsRaw
    ...
    // Free plan: throttle to once per 5 min to limit upstream Sentry API spend.
  ...
  async function pollIntegration(integ: typeof integrations[number]) {
    ...
    const newAlerts: Omit<NewAlert, "projectId">[] = await pollSentry(
      token,
      org,
      alertConfig as SentryAlertConfig,
      lookbackMinutes
    );
  ...
  // Concurrency-limited fan-out (10 parallel) to bound upstream Sentry API calls.
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (result.status === "fulfilled") {
      ...
      errors.push(`sentry/${integ.id}: ${errMsg}`);
  ...
  await pingCronHealth("poll_sentry", errors.length === 0);
