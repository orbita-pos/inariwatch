...
export async function GET(req: Request) {
  ...
  const integrationsRaw = await db
    ...
    .where(
      and(
        ...
        eq(projectIntegrations.service, "uptime")
      )
    );
  ...
  async function pollIntegration(integ: typeof integrations[number]) {
    ...
    const newAlerts: Omit<NewAlert, "projectId">[] = await pollUptime(
      endpoints,
      alertConfig as UptimeAlertConfig
    );
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (result.status === "fulfilled") {
      ...
      errors.push(`uptime/${integ.id}: ${errMsg}`);
  ...
  await pingCronHealth("poll_uptime", errors.length === 0);
