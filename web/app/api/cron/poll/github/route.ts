...
export async function GET(req: Request) {
  ...
  const integrations = integrationsRaw
    ...
    // Free plan: throttle to once per 5 min to limit upstream API spend.
  ...
  async function pollIntegration(integ: typeof integrations[number]) {
    ...
    const newAlerts: Omit<NewAlert, "projectId">[] = await pollGitHub(
      token,
      owner,
      alertConfig as GithubAlertConfig
    );
  ...
  // Concurrency-limited fan-out (10 parallel) to bound upstream GitHub API calls.
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (result.status === "fulfilled") {
      ...
      errors.push(`github/${integ.id}: ${errMsg}`);
  ...
  await pingCronHealth("poll_github", errors.length === 0);
