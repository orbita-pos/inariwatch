...
export async function GET(req: Request) {
  ...
  const integrationsRaw = await db
    ...
    .where(
      and(
        ...
        eq(projectIntegrations.service, "npm")
      )
    );
  ...
  async function pollIntegration(integ: typeof integrations[number]) {
    ...
    const newAlerts: Omit<NewAlert, "projectId">[] = await pollNpmAudit(
      {
  ...
  // Concurrency-limited fan-out (10 parallel) to bound upstream OSV.dev calls.
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (result.status === "fulfilled") {
      ...
      errors.push(`npm/${integ.id}: ${errMsg}`);
  ...
  await pingCronHealth("poll_npm", errors.length === 0);
