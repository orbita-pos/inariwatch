...
export async function GET(req: Request) {
  ...
  const integrations = integrationsRaw
    ...
    // Free plan: throttle to once per 5 min to limit upstream Vercel API spend.
  ...
  async function pollIntegration(integ: typeof integrations[number]) {
    ...
    const newAlerts: Omit<NewAlert, "projectId">[] = await pollVercel(
      token,
      teamId,
      alertConfig as VercelAlertConfig,
      lookbackMinutes
    );
  ...
  // Concurrency-limited fan-out (10 parallel) to bound upstream Vercel API calls.
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (result.status === "fulfilled") {
      ...
      errors.push(`vercel/${integ.id}: ${errMsg}`);
  ...
  await pingCronHealth("poll_vercel", errors.length === 0);
