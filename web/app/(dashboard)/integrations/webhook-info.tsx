...
export function WebhookInfo({
  ...
  return (
    <div className="mt-2 rounded-lg border border-[#1a1a1a] bg-[#080808] overflow-hidden">
      <button
        type="button"
        onClick={() => setOpen(!open)}
        className="flex w-full items-center gap-2 px-3 py-2 text-xs text-zinc-500 hover:text-zinc-300 transition-colors"
      >
      ...
      {open && (
        <div className="border-t border-[#1a1a1a] px-3 py-2.5 space-y-2.5">
          {/* URL */}
          <div>
            <p className="text-xs text-zinc-600 mb-1">Webhook URL</p>
            <div className="flex items-center gap-1.5">
              <code className="flex-1 truncate rounded bg-[#111] border border-[#1e1e22] px-2 py-1 text-xs text-zinc-400 font-mono">
                {webhookUrl}
              </code>
              <button
                type="button"
                onClick={() => copy(webhookUrl, setCopiedUrl)}
                className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-zinc-600 hover:text-zinc-300 hover:bg-white/[0.06] transition-colors"
                title="Copy URL"
              >
