...
import { db, alerts, projects, alertComments, users, apiKeys, remediationSessions, projectIntegrations, organizations, organizationMembers, substrateRecordings } from "@/lib/db";
...
import { GenerateTestPanel } from "./generate-test-panel";
...
export default async function AlertDetailPage({
  ...
  const [userPlanRows, aiKeyRows, githubRows, sentryRows, remediationRows, commentsRaw, recordingRows] = await Promise.all([
    ...
    // Inari Guard: look up any Substrate recording attached to this alert
    // so the test panel can offer "generate from recording" mode.
    db.select({
      recordingId: substrateRecordings.recordingId,
      eventCount: substrateRecordings.eventCount,
    })
      .from(substrateRecordings)
      .where(eq(substrateRecordings.alertId, alert.id))
      .orderBy(desc(substrateRecordings.createdAt))
      .limit(1),
  ]);
  const substrateRecording = recordingRows[0] ?? null;
  ...
  return (
    <div className="mx-auto max-w-[780px] space-y-5">
      ...
      {/* ── Inari Guard: regression test from this alert ─────────────────── */}
      {!alert.sourceIntegrations.includes("agent") && hasAIKey && (
        <GenerateTestPanel
          alertId={alert.id}
          projectId={alert.projectId}
          suggestedPath={extractTopStackFile(alert.body)}
          recordingId={substrateRecording?.recordingId ?? null}
          recordingEventCount={substrateRecording?.eventCount ?? null}
        />
      )}
...
/**
 * Pull the first non-vendor file path from an alert's body. Used by
 * GenerateTestPanel to pre-fill the suggested path. Tolerant of
 * V8/Node, Python, and Go stack formats. Returns null when no usable
 * frame is found.
 *
 * Examples handled:
 *   "at getUser (src/auth.ts:42:10)"            → "src/auth.ts"
 *   'File "app/auth.py", line 42, in getUser'   → "app/auth.py"
 *   "\\tsrc/auth.go:42 +0x..."                   → "src/auth.go"
 *
 * Skips node_modules, /.cargo/, runtime/, <anonymous>, and common
 * Next.js / Webpack internal paths.
 */
function extractTopStackFile(body: string | null | undefined): string | null {
  if (!body) return null;
  const skipPatterns = [
    /node_modules\//,
    /\/\.cargo\//,
  ...
  // V8: `at funcName (path:line:col)` or `at path:line:col`
  const v8Re = /at\s+(?:\S+\s+\()?([^):\n]+\.[a-z]{1,6}):\d+/g;
  let m: RegExpExecArray | null;
  while ((m = v8Re.exec(body)) !== null) {
    const file = m[1].replace(/^.*node_modules\/[^/]+\//, "");
    if (skipPatterns.some((p) => p.test(file))) continue;
    return file;
  }

  // Python: `File "path.py", line N, in funcName`
  const pyRe = /File "([^"]+\.py)", line \d+/g;
  while ((m = pyRe.exec(body)) !== null) {
    if (skipPatterns.some((p) => p.test(m![1]))) continue;
    return m[1];
  }

  // Go: bare file.go:line in panic output
  const goRe = /\b([\w./-]+\.go):\d+/g;
