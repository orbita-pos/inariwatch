...
let mounted = false;
let cleanup = null;
export function mountFeedbackWidget(options, onSubmit) {
    if (mounted)
        return cleanup ?? (() => { });
    ...
    const overlay = modal.querySelector(`[${ATTR}="overlay"]`);
    const form = modal.querySelector(`[${ATTR}="form"]`);
    const closeBtn = modal.querySelector(`[${ATTR}="close"]`);
    const cancelBtn = modal.querySelector(`[${ATTR}="cancel"]`);
    const screenshotBtn = modal.querySelector(`[${ATTR}="screenshot-btn"]`);
    const screenshotPreview = modal.querySelector(`[${ATTR}="screenshot-preview"]`);
    const statusEl = modal.querySelector(`[${ATTR}="status"]`);
    const submitBtn = modal.querySelector(`[${ATTR}="submit"]`);
    ...
    async function takeScreenshot() {
        ...
        try {
            ...
            const ImageCaptureCtor = (typeof window !== "undefined"
                ? window.ImageCapture
                : undefined);
            let bitmap;
            if (ImageCaptureCtor) {
...
// ── Styling ───────────────────────────────────────────────────────────────
function widgetCss(accent, position) {
...
function modalHtml(title, userEmail) {
