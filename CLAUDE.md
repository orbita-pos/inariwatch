...
- **AI:** Platform-funded via Together AI (`PLATFORM_TOGETHER_KEY`, `INARI_TOGETHER_PLATFORM_FUNDED_ENABLED=true`) — best-model-per-role across 8 buckets / 5 distinct models, IDs verified against Together's serverless catalog 2026-05-14. **CHAT_ORCH** (`chat.conversational`, `chat.code`) → `moonshotai/Kimi-K2.6` $1.20/$4.50 (purpose-built for multi-turn tool orchestration with thinking-mode; replaces Qwen3-235B which hit silent-tool-call + lost-context regressions on the chat surface). Inari Live no longer routes through CHAT_ORCH (pure-slash refactor 2026-05-15 made the dock input a slash-only command bar — the only AI surface there is autocomplete on the ASSIST bucket); CHAT_ORCH is retained for the web dashboard chat (`web/app/api/chat/route.ts`) and the classify endpoint. **ANALYSIS** (security/self review, risk assess) → `Qwen/Qwen3-235B-A22B-Instruct-2507-tput` $0.20/$0.60 (thinking via `chat_template_kwargs.enable_thinking`). **CLASSIFY** (intent-classify, alert.classify, code.fingerprint) → `openai/gpt-oss-20b` $0.05/$0.20. **APPLY** (`code.apply.diff`, `notify.compose.*`, `redact.pii.*`) → `Qwen/Qwen3.5-9B-FP8` $0.10/$0.15 (reused weights, separate semantic bucket). **ASSIST** (`chat.suggest-command` — Inari Live pure-slash autocomplete) → `Qwen/Qwen3.5-9B-FP8` $0.10/$0.15 (~$0.0001/call at the target 500-in/50-out ratio). **ALERT** (`alert.auto-analyze`, `alert.correlate`) → `Qwen/Qwen3.5-9B-FP8` $0.10/$0.15. **CODE** (postmortem, explore) → `Qwen/Qwen3-Coder-Next-FP8` $0.50/$1.20. **FIX** (`code.fix.*`) → `Qwen/Qwen3.6-Plus` $0.50/$3.00 (78.8% SWE-bench). Buckets defined in `web/lib/ai/together-routing.ts`; mirrors the per-role discipline of Continue.dev / opencode / Aider / Claude Code. `PLATFORM_AI_KEY` no longer used for new fixes. Together override only applies to platform-funded users (no BYOK); BYOK users use their own provider. Optional BYOK — Claude, OpenAI, Groq, Grok, DeepSeek, Gemini, Together (7 providers). MCP uses sampling-first (client LLM does analysis)
- **Deploy:** Hetzner CPX21 (Ashburn, `87.99.153.227`) via Kamal 2 + GitHub Actions (`.github/workflows/deploy-web.yml` triggers on push to `main` with `web/**` changes); Cloudflare in front
...
- **Rate limiting:** Redis-first with DB fallback — ~1ms
- **Redis:** Self-hosted on Hetzner — rate limiting, AI cache, dedup, service health, Slack cache, remediation state, job queue
- **Cron:** Go scheduler on Hetzner (replaces cron-job.org) — 5 jobs, same web app routes, Bearer auth
...
- **Security Scanning:** eslint-plugin-security (17 rules) + 19 regex patterns + AI review — runs in-memory serverless, no CLI
- **Dogfooding:** InariWatch monitors itself via `@inariwatch/capture/next` wrapper + `instrumentation.ts`

## Hetzner infrastructure — 3 servers

Three distinct physical hosts. The "Hetzner" name in env vars / commit
messages does NOT refer to a single box.

| Server | Location | IP / host | Managed by | Services |
|---|---|---|---|---|
| **inari-web** | Ashburn (US-East) | `87.99.153.227` (CPX21, 2 vCPU / 4GB) | Kamal 2 (`web/config/deploy.yml`) | Next.js app on `app.inariwatch.com` + 6 marketing/api hostnames, kamal-proxy TLS, Redis accessory (port 6379), MCP server (rewrite from `mcp.inariwatch.com`), inariwatch-agent eBPF |
| **inari-staging** | Ashburn (US-East) | URL in sops (`STAGING_SERVER_URL` / `WORKER_URL`) — CX22 class | systemd + Docker, **NOT Kamal** | Go staging server (port 9400, container lifecycle + cron scheduler), Node.js AI worker (port 9401, container-agent AI loop), Redis (6379 — separate from web's Redis), Caddy (443, TLS + routing `/worker/*` → 9401 / rest → 9400) |
| **inari-ml** | Falkenstein (DE) | `178.105.21.94:9410` (`INARI_ML_URL`) | Docker directo, **NOT Kamal** | Python FastAPI sidecar: HST anomaly scoring + ADWIN drift + Drain3 log-template clustering + SNARIMAX per-workspace forecast + GLiNER NER on stack traces. Pickle persistence at `/data/ml_models.pkl` (schema-versioned); Drain3 state at `/data/drain3_state.bin` |

**Updating each server:**
- `inari-web` → `cd web && kamal deploy` (or `kamal env push` for env-only changes)
- `inari-staging` → ssh + systemd reload (manual; `inari-staging` and `inari-worker` are separate systemd units on the same host)
- `inari-ml` → `scp ml-sidecar/main.py root@178.105.21.94:/opt/inari-ml/ && ssh root@... 'docker restart inari-ml'`

**Cron scheduler** (built into Go server on inari-staging): 5 jobs trigger web app routes with `Bearer CRON_SECRET`. Replaces cron-job.org.
...
**Redis caching layer** (self-hosted on Hetzner):
- Rate limiting: sliding window (~1ms vs 30-50ms DB)
...
- Service health: shared across web instances via `HSET`
...
- **Alerts** — ingest from Sentry, Vercel, GitHub, Datadog, Expo, @inariwatch/capture via webhooks; free AI auto-analysis on arrival (Qwen3.5-9B via Together AI, no BYOK needed)
...
| Module | File | Purpose |
|---|---|---|
| AI client | `client.ts` | Multi-provider dispatcher (7 providers: OpenAI, Claude, Groq, Grok, DeepSeek, Gemini, Together) |
...
| Redis client | `lib/redis.ts` | Redis client (rate limiting, AI cache, dedup, service health, Slack cache) |
...
- **Worker mode** (`WORKER_URL` set): AI loop runs on Hetzner Node.js worker (`worker/src/`), 40 turns, Docker on localhost (~1ms tool calls), no serverless timeout. ~$0.25/fix with prompt caching.
...
- `PLATFORM_TOGETHER_KEY` — Together AI key (primary platform key): Qwen3.5-9B-FP8 (alerts), Qwen3-Coder-Next-FP8 (postmortem/explore), Qwen3-235B-A22B-Instruct-2507-tput (chat/review/risk), Qwen3.6-Plus (code fix). Full Qwen stack — IDs verified against Together serverless catalog 2026-05-13. Requires `INARI_TOGETHER_PLATFORM_FUNDED_ENABLED=true`. Only applies to platform-funded users (no BYOK).
- `PLATFORM_AI_KEY` — OpenAI key: legacy, no longer used for platform-funded fixes (migrated to Qwen3.6-Plus). Still read for BYOK OpenAI users.
- `PLATFORM_ANTHROPIC_KEY` — Claude key: available for Claude-specific paths; largely superseded by Together routing
- `RESEND_API_KEY` — Resend email service
- `ADMIN_EMAIL` — grants access to `/admin`
- `APP_URL` — used for cron fan-out
...
- `REDIS_URL` — Redis connection string (self-hosted on Hetzner)
...
Kernel-level observability agent. Installed via `curl -sf https://install.inariwatch.com | sh`. Zero code changes, language-agnostic.

**Stack:** C (eBPF, ~10 files) + Rust (userspace, 95% of code) + libbpf-rs/libbpf-cargo v0.26 (CO-RE) + blazesym v0.2 (symbolization) + libbpf-async (tokio ring buffer). NOT Aya (CO-RE incomplete on rustc).

**7 probes active on Hetzner kernel 6.8.0:** process (tracepoint/sched), network (tracepoint/sock + kprobe/tcp_sendmsg), filesystem (kprobe/vfs_*), dns (kprobe/udp_sendmsg, parsed in Rust), syscall (raw_tracepoint/sys_enter), tls (uprobe SSL_read/SSL_write, scans /proc/PID/maps every 30s), security (BPF LSM via GRUB).

**Pipeline:** BPF ring buffer → crossbeam channel → tokio workers → batch (1000/256KB/5s) → LZ4 → HTTPS. CO-RE single static binary for kernels 5.8+. Graceful degradation per probe (process is the only fatal one). Observed: 248 events/sec, ~88% compression, <1% CPU.

**Cloud endpoint:** `POST /api/agent/events` (`web/app/api/agent/events/route.ts`), Bearer auth (HMAC-SHA256 of agent_id with integration webhook secret), threat detection in `web/lib/agent/processor.ts`, feeds existing `createAlertIfNew()` pipeline.

**TODO:** wire to cloud DSN/integration/secret, install as systemd, release via GitHub Actions CI.

## Inari Live — Phase 5 (Interactive Slot Filling + Scoped Memory)

Phases 1–4 of the pure-slash refactor (SHIPPED 2026-05-15) made the dock input a slash-only command bar with AI autocomplete on Qwen3.5-9B. Phase 5 (SHIPPED 2026-05-15) layers two new capabilities on top:

- **SuspendedCommand protocol** — slash handlers can return `{kind: "suspended", needs, partial, rebuild}` instead of erroring when an arg is missing. The dock surface renders an inline `<SlotPicker>` (one slot at a time, sequential); on pick, `resumeSlashCommand` re-runs the handler with merged args. Five `SlotKind`s — `contact`, `project`, `alert`, `path`, `text` — each with a specialised picker in `desktop/src/components/dock/slots/`. Cancel always returns the dock to idle.
- **Scoped memory** — in-memory ring buffer (cap 3) in `desktop/src/lib/slash/scoped-memory.ts`. `/alerts` and `/projects` record structured entities; autocomplete forwards `toAutocompletePromptContext()` to the web endpoint as `memoryContext` (placed adjacent to the user query per Lost-in-the-Middle). Bilingual reference resolver (es+en): "esa alerta", "el último proyecto", "the first" all resolve deterministically to entities in memory. AlertPickerSlot + ProjectPickerSlot promote memory entries above the fresh cloud list with a "recent" chip.

4 pilot commands flip to suspend-aware in Phase 5: `/whatsapp` (contact + text), `/install` (path; preserves `--project=` flag), `/fix` (new — alert), `/health` (project). The other 17 commands keep their pre-Phase-5 error-note path; Phase 6 rolls them out mechanically.

Prompt-cacheable invariants:
- `buildCacheableUserPromptPrefix(manifest)` is byte-identical across different `memoryContext` values — manifest + few-shot stay cacheable, memory is fresh-per-request. Cache key in `/api/ai/suggest-slash` keys on memoryContext so different memory = different cache entry.
- Memory entry summaries (Phase 5.3 mini-diff) inline `inari:alert:<hash>` refs + per-entity discriminator (title + HH:MM) so the LLM can resolve free-form references the regex resolver can't catch ("fixea la del payment"). Capped at 5 entries + "(+N more)" tail.

Plan + SHIPPED commit table: `INARI_LIVE_PHASE_5_PLAN.md`. Memory entry: `project_inari_live_phase_5.md`.
...
| Area | CLI | Web | Why |
|------|-----|-----|-----|
| **Default AI model** | `claude-haiku-4-5-20251001` (fast, cheap) | Qwen3.5-9B (alerts), Qwen3-235B-A22B (chat/review/risk), Qwen3-Coder-Next (postmortem/explore), Qwen3.6-Plus (code fix — single-shot + agent-loop + container) | CLI runs locally, cost-sensitive; Web is 100% Qwen stack via Together AI — zero OpenAI for platform-funded users (IDs verified 2026-05-13) |
