...
export const ABSOLUTE_MAX_BODY_BYTES = 500000;
 ...
 */
export function urlIsDenied(url, customerPatterns = []) {
    if (!url)
        return true;
...
export function contentTypeIsCapturable(contentType) {
    if (!contentType)
        return false;
 ...
 */
export function maskSecretsInJson(value, depth = 0) {
    if (depth > 8)
        return "[depth-limit]";
    if (value === null || value === undefined)
        return value;
    if (Array.isArray(value))
        return value.map((v) => maskSecretsInJson(v, depth + 1));
...
export function processBody(opts) {
    ...
    if (!raw)
        return null;
    if (!contentTypeIsCapturable(contentType))
        return null;
...
//# sourceMappingURL=network-body.js.map