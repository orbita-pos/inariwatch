...
use super::types::{AIMessage, AIResponse, AITool, AIUsage, ChatChunk, ToolCallDelta};
...
impl OpenAICompatAdapter {
    ...
    pub fn for_provider(provider: AIProvider) -> Self {
        match provider {
            ...
            AIProvider::Together => OpenAICompatAdapter {
                provider,
    ...
    /// (`:` lines) are skipped.
    ///
    /// `tools` is the S6 chat-agent catalog. When non-empty the request
    /// body grows a `tools: [{type:"function", function: ...}]` array
    /// and SSE chunks may carry `delta.tool_calls[]`, surfaced as
    /// [`ChatChunk::tool_call`].
...
fn serialize_tools(tools: &[AITool]) -> Value {
    Value::Array(
        tools
            .iter()
            .map(|t| {
...
fn parse_frame(frame: &[u8]) -> Result<Option<ChatChunk>, ProviderError> {
    ...
    if let Some(choice) = parsed.choices.into_iter().next() {
        ...
        // OpenAI emits one `delta.tool_calls[]` entry per active call
        // per chunk. We surface only the first entry — concurrent tool
        // calls in one assistant turn arrive in *separate* chunks each
        // with one entry, distinguished by `index`. The accumulator in
        // desktop's `stream_to_bus` keys on `index` to assemble the
        // full call regardless of interleaving.
        if let Some(tc) = choice.delta.tool_calls.and_then(|mut v| v.drain(..).next()) {
...
struct ChunkDelta {
    #[serde(default)]
    content: Option<String>,
    /// S6 — populated when the model emits a function/tool call. One
    /// entry per `index`; OpenAI streams these incrementally so the
    /// arguments string assembles across chunks.
    #[serde(default)]
