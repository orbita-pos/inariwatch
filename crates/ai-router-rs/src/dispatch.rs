...
use crate::providers::types::{AIMessage, AIResponse, AITool, ChatChunk};
...
pub struct DispatchInput<'a> {
    ...
    /// S6 — chat-agent tool catalog. Empty slice keeps the legacy
    /// no-tool behavior; populated for the desktop dock chat surface
    /// only (Inari Live S6). Wired into the OpenAI direct-mode adapter;
    /// proxy mode + Anthropic + Llama.cpp ignore it for now.
    pub tools: &'a [AITool],
}
