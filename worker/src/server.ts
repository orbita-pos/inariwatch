...
import { runTest, type RunTestRequest } from "./runtest/handler.js";
...
// ── Routes: Inari Guard run_test ───────────────────────────────────────────

/**
 * POST /worker/run_test
 *
 * Verifies a generated test by cloning the repo, installing deps, and
 * running the framework against just that file. Same concurrency-cap
 * pattern as whatif/screenshot — npm install is the expensive bit.
 *
 * Body: see RunTestRequest in `runtest/handler.ts`.
 * Auth: Bearer STAGING_API_SECRET (checked above in the server handler).
 */
let activeRunTestJobs = 0;
const MAX_RUNTEST_CONCURRENT = Number(process.env.MAX_RUNTEST_CONCURRENT ?? 2);

async function handleRunTest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  if (activeRunTestJobs >= MAX_RUNTEST_CONCURRENT) {
    json(res, 503, {
      error: `run_test worker at capacity (${activeRunTestJobs}/${MAX_RUNTEST_CONCURRENT}). Retry shortly.`,
      code: "capacity",
    });
    return;
  }
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    } else if (req.method === "POST" && path === "/worker/run_test") {
      await handleRunTest(req, res);
