...
#### Done (2026-05-01) — Sesión 12 ✨ Track 3 cerrado 100%

- **Spec note (read first):** HANDOFF originally specified subscription to `FixApplied` / `FixRejected` / `RegressionDetected` with `repo_id` + `fingerprint` on the wire. Actual implementation subscribes to `RemediationCompleted` + `FixRejected` (the existing event shapes from S19) and joins event → `remediation_sessions` row in store for the `(repo_id, fingerprint)` pair. Semantic equivalence preserved; the nomenclature delta is documented in DECISIONS 2026-05-01 ("Sesión 12 — Path A: store-lookup adaptation"). `RegressionDetected` is deferred entirely — no emitter exists today; the `apply_failure` helper accepts a `forced_demote: bool` flag covered by a unit test so the path is correct when the emitter ships.
- **Branch:** `feat/inari-live-track3-session12-procedural-patterns` (off S20 tip `88b6716`).
- **Tip commit:** NOT YET COMMITTED — change set staged in working tree, awaiting architect ack before squash. `git status` shows 6 modified + 5 added (resources/schemas dir + 2 new procedural mods + 3 new integration tests). NOT pushed (cadence rule + the prompt explicitly forbids it).
- **Outputs (added):**
  - `desktop/src-tauri/src/memory/procedural/learner.rs` — `apply_success` / `apply_failure` / `prune_if_needed` / `load_patterns` / `save_patterns` / `patterns_path` / `spawn_pattern_learner`. In-memory `Arc<RwLock<HashMap<RepoId, PatternsFile>>>` write-through cache. Match arms over `DaemonEvent` are exhaustive (no `_`) — listed ignored variants explicitly. 9 unit tests covering all mutation paths + corrupt/missing/future-version load failures.
  - `desktop/src-tauri/src/memory/procedural/matcher.rs` — `match_patterns(store, repo_id, fingerprint, opts)` + sync `rank_patterns` helper for testability + `MatchOptions::default() = (max_age_days=90, top_k=5, include_anti_patterns=true)`. Ranking is `success_rate * exp(-age_days / 30)` (30-day recency half-life). 6 unit tests.
  - `desktop/src-tauri/resources/schemas/patterns.schema.json` — JSON Schema draft-07 doc for `.inari/patterns.json` (informational; no runtime validation).
  - `desktop/src-tauri/tests/procedural_learns_success_pattern.rs` — 3× `RemediationCompleted{success:true}` for the same fingerprint → `success_count == 3`, kind `auto-detected`, evidence preserved in order, exactly 3× `PatternLearned` published.
  - `desktop/src-tauri/tests/procedural_failure_majority_demotes.rs` — 1× success + 5× `FixRejected` for the same fingerprint → `kind == anti-pattern`, `failure_count == 5`, exactly 1× `PatternDemoted` (`reason="failure_majority"`, `prior_success=1`, `new_failure=2` — the demote moment). Replaces the prompt's `procedural_demotes_to_anti_pattern.rs` (renamed to drop the `RegressionDetected` half).
  - `desktop/src-tauri/tests/procedural_matcher_ranks_top_k.rs` — 3 tests: top-5 truncation + anti-pattern exclusion under `include_anti_patterns=false`; recency overrides perfect success rate (fresh 0.9 beats 60-day-old 1.0 ≡ 0.9 > 0.25); save → load → save byte-identical roundtrip.
- **Outputs (modified):**
  - `desktop/src-tauri/src/daemon/mod.rs` — added `DaemonEvent::PatternLearned { repo_id, fingerprint, kind, success_count }` + `DaemonEvent::PatternDemoted { repo_id, fingerprint, prior_success, new_failure, reason }`. `kind` field on `PatternLearned` carries the wire string `"auto-detected"`/`"anti-pattern"` and uses the `#[serde(rename = "pattern_kind")]` workaround the rest of the internally-tagged variants use (FsChange/GitEvent/MemoryReviewRequested precedent).
  - `desktop/src-tauri/src/memory/episodic/mod.rs` — `kind_tag` arms: `PatternLearned` → `"pattern_learned"`, `PatternDemoted` → `"pattern_demoted"` (both persisted, audit-relevant — infinite TTL via the retention runner's `_ => Infinite` default). `repo_id_tag` arms scrape the `repo_id` field off both new variants.
  - `desktop/src-tauri/src/memory/declarative/mod.rs` — `atomic_write` generalised: was `with_extension("md.tmp")` (silently broken for non-md files), now appends `.tmp` to the existing extension (`memory.md` → `memory.md.tmp` unchanged; `patterns.json` → `patterns.json.tmp` correct). Reused by `procedural::learner::save_patterns` per the architect's "no fork" instruction.
  - `desktop/src-tauri/src/memory/procedural/mod.rs` — replaced the S12 stub. Public types: `PatternKind` (kebab-case serde: `auto-detected`/`anti-pattern`), `Pattern`, `PatternsFile`, plus the `MAX_PATTERNS_PER_REPO=500` / `PRUNE_BATCH=50` / `MAX_EVIDENCE_PER_PATTERN=50` / `CURRENT_VERSION=1` constants.
  - `desktop/src-tauri/src/store/queries.rs` — added `get_remediation_session_meta(store, id) -> Option<(repo_id, fingerprint)>` (2-column projection used by the learner; full 15-column `get_remediation_session` would be wasteful on every event).
  - `desktop/src-tauri/src/lib.rs` — wired `memory::procedural::spawn_pattern_learner(daemon, store)` into the Tauri setup hook, AFTER `spawn_event_persister` (the persister must subscribe BEFORE the learner publishes so the audit `PatternLearned` / `PatternDemoted` rows are picked up).
- **Behavior shipped:**
  - Procedural learner subscribes to the bus and routes `RemediationCompleted{success:true}` → `apply_success`, `RemediationCompleted{success:false}` + `FixRejected` → `apply_failure`. All other variants are explicitly enumerated as ignored arms (no `_`) so a future `DaemonEvent` variant forces a build-time classify-or-skip decision.
  - `.inari/patterns.json` is the SSOT — commit-friendly, 2-space indent, integer-only fields. Stable key order via struct declaration order in `Pattern` + `PatternsFile`. Save → load → save is byte-identical (covered by integration test).
  - Anti-pattern rows are exempt from pruning (forgetting an anti-pattern would let a known-bad fix re-surface — strictly worse than the few KB of disk).
  - Anti-pattern rows do NOT auto-revert on later success (`apply_success` keeps the kind; only an explicit human signal — out of scope for v0.1 — can re-promote).
  - Corrupt / future-version `patterns.json` → `SensorWarning` on the bus + start with empty file. Same posture as the FS sensor's inotify-saturation path.
  - Missing `remediation_sessions` row for a `session_id` → `SensorWarning` + skip event (not a crash).
- **Confirmations for the architect:**
  - `cargo check --lib` clean (10.06s after 1.88s cached check; cold path was hit once during the daemon-event extension to surface the `kind` ↔ internal tag conflict that drove the `pattern_kind` rename).
  - `cargo check --lib --tests` clean (2.21s warm).
  - All match arms exhaustive: episodic `kind_tag` + `repo_id_tag` + learner's bus loop. No `_ =>` in any of the three.
  - No new Cargo deps. `tempfile` (used in the unit + integration tests) was already in `[dev-dependencies]` from S5/S11.
  - No frontend changes — pure backend session as specified.
  - No IPC commands — matcher is internal API; expose when S19/single_shot.rs consumes it + UI has a place to render.
  - `RegressionDetected` deferred (DECISIONS 2026-05-01); `forced_demote=true` path is exercised by the unit test `forced_demote_overrides_count_majority` so the helper API is correct when the future emitter lands.
- **Tests run 2026-05-01 (all `RUSTFLAGS="-C debuginfo=1"`):**
  - `cargo test --test procedural_matcher_ranks_top_k -- --test-threads=1` — **3/3 pass** (1m 12s cold link / 0.99s test runtime; 0.59s warm).
  - `cargo test --test procedural_learns_success_pattern -- --test-threads=1` — **1/1 pass** (40.76s incremental / 0.40s test runtime; 0.41s warm).
  - `cargo test --test procedural_failure_majority_demotes -- --test-threads=1` — **1/1 pass** (5.20s incremental / 0.79s test runtime; 0.42s warm).
  - `cargo test --lib memory::procedural -- --test-threads=1` — **16/16 pass** (21.12s incremental / 0.01s test runtime). Covers: `apply_success` increment + first-summary-wins, `evidence_capped` (oldest-drop at MAX_EVIDENCE_PER_PATTERN), `failure_majority_demotes`, `forced_demote_overrides_count_majority`, `anti_pattern_extra_success_does_not_revert`, `prune_keeps_anti_patterns` (600 patterns → 550 retained including all 100 anti-patterns), `save_load_roundtrip_byte_identical`, `load_missing_file_returns_empty`, `load_future_version_is_parse_error`, `load_corrupt_file_is_parse_error`, `anti_pattern_excluded_when_opted_out`, `anti_pattern_included_by_default`, `fresh_high_rate_outranks_stale_perfect`, `max_age_filters_old_patterns`, `fingerprint_filter`, `top_k_truncates`.
  - **Regression** — `cargo test --test episodic_append_and_query -- --test-threads=1` — **1/1 pass** (10.56s incremental / 0.56s runtime). Confirms the `kind_tag` / `repo_id_tag` extension didn't break the S13 invariant ("exactly 3 rows persist").
- **Track 3 status:** **CERRADO 100%.** Sesión 11 (declarative) + Sesión 13 (semantic + episodic + retention) + Sesión 12 (procedural) all done. The 4-layer memory stack is wired end-to-end.
- **Disk:** Started at 1.2 GB free → cleaned stray `desktop/src-tauri/target/` (1.7 GB, per-crate target that doesn't respect `CARGO_TARGET_DIR`) + `~/.cargo/target-shared/debug/incremental` (341 MB) + shared `flycheck0` (236 KB) → 3.1 GB. Cleared incremental once mid-run after disk dropped to 1.7 GB. Final: 2.7 GB free.
- **Notes for downstream Tracks:**
  1. **The matcher is not yet wired into S19's `single_shot.rs`.** Do that in the first "use procedural recall" session — the call shape is `memory::procedural::match_patterns(&store, repo_id, error_fingerprint, MatchOptions::default()).await`. Use the result to enrich the AI prompt with "the fix that worked N times for this fingerprint was: <summary>".
  2. **`RegressionDetected` reopen condition:** when post-merge regression detection ships (likely Track 6 or post-launch), add the variant to `DaemonEvent`, classify it in episodic `kind_tag`, and route it in the learner's bus loop to `apply_failure(..., forced_demote=true)`. The `apply_failure` helper already accepts the flag; the unit test `forced_demote_overrides_count_majority` proves the path is correct.
  3. **`PatternLearned` audit rows are infinite-TTL via the `_ => Infinite` default in `retention::ttl_for`.** If you want explicit TTL entries (clarity over implicit defaults), add `"pattern_learned" => EventTtl::Infinite` + `"pattern_demoted" => EventTtl::Infinite` arms to `retention::ttl_for` and append them to `MANAGED_KINDS`. Behaviour is identical either way.
  4. **The architect-approved Path A means cross-track changes (renaming `RemediationCompleted` to `FixApplied` + adding `repo_id`/`fingerprint` to the wire format) are NOT done.** Future external consumers (mobile, web sidecar) reading the bus without store access would need either (a) a new variant carrying the FK fields, or (b) a small store-lookup adapter on the consumer side. Not a v0.1 concern.
  5. **No IPC surface.** The frontend has no path to query patterns yet. The natural seam is a Tauri command in `ipc/memory.rs` once the dock has a place to render "47 teams fixed this" cards.
