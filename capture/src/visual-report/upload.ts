...
export interface UploadInput {
  ...
  screenshot:   string;        // data: URI from the Screen Capture API helper
  description:  string;
...
export async function uploadVisualReport(input: UploadInput): Promise<UploadResult> {
  const env       = readEnv();
  ...
  const body = JSON.stringify({
    ...
    payloadSize: undefined as number | undefined, // server measures actual bytes
  });
  ...
  try {
    const res = await fetch(endpoint, {
      ...
      keepalive: body.length < 60_000, // browser keepalive cap
    });
    ...
    const json = await res.json() as { ok?: boolean; reportId?: string; alertId?: string };
    return {
      ok:        json.ok !== false,
      status:    res.status,
...
function readEnv(): {
  HOST?: string; TOKEN?: string; PROJECT_ID?: string;
} {
...
function resolveAuth(config: CaptureConfig, env: ReturnType<typeof readEnv>): ResolvedAuth {
  ...
  if (config.dsn) {
    try {
      ...
      if (u.username) dsnToken = decodeURIComponent(u.username);
