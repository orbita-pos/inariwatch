...
export type ConsoleEntry = {
  ...
  ts:    number;
  args:  unknown[];
  site:  string | null; // top frame of new Error().stack at call time
};
...
export function readConsoleRing(): ConsoleEntry[] {
...
export function installNetworkRing(): void {
  ...
  if (typeof window.fetch === "function") {
    ...
    window.fetch = async (input, init) => {
      ...
      const reqMethod = typeof input !== "string" ? (input as Request).method : undefined;
  ...
  if (typeof window.XMLHttpRequest === "function") {
    ...
    proto.open = function patchedOpen(
      this: XMLHttpRequest & { __iwUrl?: string; __iwMethod?: string },
      method: string,
      url: string | URL,
      ...rest: unknown[]
    ): void {
  ...
  function push(entry: NetworkEntry) {
    ...
    if (networkRing.length > NETWORK_RING_CAP) networkRing.shift();
  }
}

export function readNetworkRing(): NetworkEntry[] {
  return networkRing.slice();
}

// ── Safe serializer (bounded depth + length) ─────────────────────────────────

function safeSerialize(v: unknown, depth = 2, maxStr = 512): unknown {
  if (depth < 0) return "[truncated]";
  if (v === null || v === undefined) return v;
  const t = typeof v;
  if (t === "number" || t === "boolean") return v;
  if (t === "string") return (v as string).slice(0, maxStr);
  if (t === "function") return "[function]";
  if (v instanceof Error) return { name: v.name, message: v.message, stack: v.stack?.slice(0, 2048) };
  if (typeof Node !== "undefined" && v instanceof Node) return `[${(v as Node).nodeName}]`;
  if (Array.isArray(v)) return v.slice(0, 20).map((x) => safeSerialize(x, depth - 1, maxStr));
  if (t === "object") {
    const out: Record<string, unknown> = {};
    const keys = Object.keys(v as object).slice(0, 30);
    for (const k of keys) {
      try {
        out[k] = safeSerialize((v as Record<string, unknown>)[k], depth - 1, maxStr);
      } catch {
