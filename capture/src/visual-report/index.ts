...
import { mountFeedbackWidget, type WidgetOptions, type FeedbackPayload } from "../feedback/widget.js";
import { installConsoleRing, installNetworkRing } from "./rings.js";
import { captureContext } from "./capture-context.js";
import { uploadVisualReport, type UploadResult } from "./upload.js";
 ...
 */
export function visualReportIntegration(options: VisualReportOptions = {}): Integration {
  return {
    name: "VisualReport",
    setup(config: CaptureConfig) {
      ...
      const mount = () => {
        mountFeedbackWidget(widgetOptions, (payload: FeedbackPayload) => {
            ...
            try {
              ...
              if (!payload.screenshot) {
                ...
                if (config.debug && !config.silent) {
                  console.warn(
                    "[@inariwatch/capture/visual-report] No screenshot attached — skipping upload. " +
              ...
              if (!result.ok && config.debug && !config.silent) {
                console.warn(
                  "[@inariwatch/capture/visual-report] Upload failed:",
                  result.status,
                  result.error,
                );
              }
