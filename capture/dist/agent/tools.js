...
export function getLocalsAtFrame(event, frameIndex) {
 ...
 */
export function evaluateInFrame(_event, _frameIndex, _expression) {
    return {
        ok: false,
        error: "evaluateInFrame: inspector.Session post-mortem eval not yet wired (see capture-forensic). Available in capture-agent >= 0.2.0.",
    };
}
export function matchFingerprint(event, fingerprint) {
 ...
 */
export function diffSinceDeploy(event) {
...
export const TOOL_SCHEMAS = [
    {
        ...
        description: "Return the captured local variables at a specific stack frame. Use this when a hypothesis depends on the value of a local variable at throw time.",
        parameters: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Evaluate an arbitrary JavaScript expression against the captured frame state. Use sparingly — only when locals alone are insufficient.",
        parameters: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Check whether other workspaces have hit this exact error fingerprint and have a community fix recorded.",
        parameters: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Get a summary of git changes between the failing deploy and the prior known-good deploy. Useful for hypothesizing regressions.",
        parameters: {
            type: "object",
            properties: {},
            required: [],
