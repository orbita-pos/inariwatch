...
import { canonicalize } from "./client.js";
...
const RELAY_TS_ACCEPT_PAST_MS = 30000;
const RELAY_TS_ACCEPT_FUTURE_MS = 5000;
export class InMemoryRelay {
    ...
    consumeToken(peerId, nowMs) {
        ...
        if (elapsed > 0) {
            const refill = (elapsed / 60000) * RELAY_RATE_LIMIT_PER_MINUTE;
...
export function isFreshTimestamp(tsIso, nowMs) {
    const ts = Date.parse(tsIso);
    if (Number.isNaN(ts))
        return false;
    if (ts > nowMs + RELAY_TS_ACCEPT_FUTURE_MS)
        return false;
    if (ts < nowMs - RELAY_TS_ACCEPT_PAST_MS)
        return false;
    return true;
}
export function isPeerIdConsistent(msg) {
    try {
...
export function verifySignatureV1(msg) {
