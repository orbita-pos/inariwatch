...
import { addBreadcrumb, clearScope, getBreadcrumbs, getRequestContext, getTags, getUser, } from "./scope.js";
...
export function init(cfg = {}) {
    ...
    if (cfg.dsn) {
        try {
            ...
            console.error(`[inariwatch-capture] DSN parse error: ${err.message} — local mode`);
        ...
        if (!cfg.silent && typeof console !== "undefined") {
            // eslint-disable-next-line no-console
            console.info("[inariwatch-capture] Local mode — errors print to console. Set dsn to send to cloud.");
        }
        transport = new LocalTransport();
    }
...
export function setTransportForTesting(t) {
    transport = t;
    inited = true;
}
...
export async function captureException(err, extra) {
    ...
    if (extra)
        ev.context = { ...ev.context, ...extra };
    await dispatch(ev);
}
export async function captureMessage(message, severity = "info") {
    if (!inited || !transport)
        return;
    const ev = await base(message, message, severity, "error");
    await dispatch(ev);
}
export async function captureLog(message, level, metadata) {
    if (!inited || !transport)
        return;
    const sev = mapLevel(level);
    const ev = await base(message, message, sev, "log");
    if (metadata)
        ev.metadata = { ...ev.metadata, ...metadata };
    await dispatch(ev);
}
...
async function base(title, body, severity, eventType) {
    const ev = {
