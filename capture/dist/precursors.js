 ...
 */
export function initPrecursors() {
    if (state)
        return;
    const s = {
        ring: [],
...
export function stopPrecursors() {
    if (!state)
        return;
    if (state.timer)
 ...
 */
export function snapshotPrecursors() {
    if (!state)
        return [];
    ...
    if (peakP99 > 0) {
        const deltaPct = baselineP99 > 0 ? ((peakP99 - baselineP99) / baselineP99) * 100 : peakP99 * 100;
...
export function recordCircuitBreakerTrip() {
    if (state)
        state.counters.circuitBreakerTrips++;
}
// ───────────────────────── Internals ───────────────────────────────────────
function tryAttachPerfHooks(s) {
    ...
    import(/* webpackIgnore: true */ pkg)
        ...
        if (typeof ph?.monitorEventLoopDelay !== "function")
            return;
        try {
            const monitor = ph.monitorEventLoopDelay({
...
function startJitterFallback(s) {
...
function tryAttachUndiciChannel(s) {
    if (typeof process === "undefined")
        return;
...
// `fulltrace.ts`.
export function __resetPrecursorsForTesting() {
    stopPrecursors();
}
export function __forceSampleForTesting() {
    if (state)
        sample(state);
}
export function __isPerfHooksActiveForTesting() {
    return !!state?.monitor;
}
export async function __waitForPerfHooksReadyForTesting(timeoutMs = 1000) {
    const start = nowMs();
...
export function __getRingForTesting() {
    return state ? readRingOrdered(state) : null;
}
//# sourceMappingURL=precursors.js.map