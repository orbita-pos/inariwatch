...
const DOC_SUBDIRS = ["", "docs", "doc", "api"];
// project root → resolved spec path | null (negative cache)
const rootToSpec = new Map();
    ...
    for (const [pathKey, pathItem] of Object.entries(paths)) {
        ...
        for (const method of HTTP_METHODS) {
            ...
            if (!op || typeof op !== "object")
                continue;
            const indexed = { method, path: pathKey, op, spec };
...
function inferPathFromFile(filePath) {
...
function pathsMatch(inferred, openapi) {
    if (inferred === openapi)
        return true;
...
function operationToShape(op) {
    ...
    const params = Array.isArray(op.op.parameters) ? op.op.parameters : [];
    if (params.length === 0)
        return null;
    const properties = {};
    const required = [];
...
function derefIfNeeded(node, spec) {
...
// `{ type: "unknown" }`.
function jsonSchemaToShape(schemaIn, spec, seen) {
    ...
    if (typeof schema.$ref === "string") {
        ...
        if (seen.has(ref))
            return { $ref: ref, _symbol: refSymbol(ref) };
    ...
    const variants = (Array.isArray(schema.oneOf) && schema.oneOf) ||
        ...
        null;
    if (variants) {
        const parts = variants.map((v) => jsonSchemaToShape(v, spec, seen));
        if (parts.every((p) => p.enum && p.enum.length === 1)) {
            return { enum: parts.flatMap((p) => p.enum) };
        }
        return { enum: parts.map((p) => p.type ?? "unknown") };
    }
...
function findFuzzyPath(byPath, target) {
...
function pickBestOp(ops, symbol) {
    if (symbol) {
        const upper = symbol.toUpperCase();
        for (const op of ops) {
            if (upper.includes(op.method.toUpperCase()))
                return op;
        }
    }
...
// ─── Test hooks ────────────────────────────────────────────────────────────
export function __resetOpenapiCacheForTesting() {
    rootToSpec.clear();
    specCache.clear();
