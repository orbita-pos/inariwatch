...
function collectDeclarations(ts, sf) {
...
function firstExportedFn(map) {
    for (const [, decl] of map) {
        const mods = decl.modifiers ?? [];
        if (mods.some((m) => m.kind && isExportKind(m.kind)))
            return decl;
    }
...
function resolveTypeNode(ts, node, env, seen) {
    ...
    if (ts.isUnionTypeNode(node)) {
        const parts = (node.types ?? []).map((t) => resolveTypeNode(ts, t, env, seen));
...
function resolveMembers(ts, members, env, seen) {
    const properties = {};
    const required = [];
    for (const m of members) {
        ...
        const child = m.type ? resolveTypeNode(ts, m.type, env, seen) : { type: "unknown" };
...
function narrowForEnum(s) {
    if (s.enum && s.enum.length === 1)
        return s.enum[0];
    return s.type ?? "unknown";
}
function textOfTypeName(ts, n) {
    if (ts.isIdentifier(n))
        return n.text;
    if (ts.isQualifiedName(n))
        return `${textOfTypeName(ts, n.left)}.${n.right.text}`;
    return "";
}
function symbolNameOf(ts, typeNode) {
    if (ts.isTypeReferenceNode(typeNode))
        return textOfTypeName(ts, typeNode.typeName) || null;
    return null;
}
function coerceLiteral(ts, lit) {
