...
export const drizzleSource = {
    ...
    extract(filePath, symbol) {
        ...
        if (env.tables.size === 0)
            return null;
        // (1) Symbol == table variable name OR runtime table name.
        if (symbol) {
            const direct = env.tables.get(symbol) ??
...
function collectDrizzle(ts, sf) {
    const tables = new Map();
    const tablesByRuntimeName = new Map();
    ...
    function visit(node) {
        ...
        if (ts.isVariableStatement(node)) {
            for (const decl of node.declarationList.declarations) {
                ...
                if (t) {
                    const entry = { varName: decl.name.text, ...t };
                    tables.set(decl.name.text, entry);
                    if (entry.runtimeName)
                        tablesByRuntimeName.set(entry.runtimeName, entry);
                    continue;
                }
...
function parseTableCall(ts, expr) {
    ...
    if (!calleeName || !TABLE_FACTORIES.has(calleeName))
        return null;
...
function walkTable(ts, table, env, seen) {
    const properties = {};
    const required = [];
    if (!table.columns) {
        return {
...
function walkColumn(ts, expr, env, seen) {
    ...
    let shape = factoryName && COLUMN_TYPES[factoryName]
...
function sawArrayModifier(ts, expr) {
    let cur = expr;
    while (ts.isCallExpression(cur) &&
        ts.isPropertyAccessExpression(cur.expression) &&
        cur.expression.name &&
        ts.isIdentifier(cur.expression.name)) {
        if (cur.expression.name.text === "array")
            return true;
        if (!ts.isCallExpression(cur.expression.expression))
            break;
        cur = cur.expression.expression;
    }
    return false;
...
function findTableTargetInFn(ts, fn) {
    let target = null;
    function visit(n) {
        if (target)
            return;
