...
function parsePrismaText(text, mtimeMs) {
...
function modelBodyToShape(modelName, body, enums) {
    const properties = {};
    const required = [];
...
// ─── Symbol resolution ─────────────────────────────────────────────────────
function resolveModelName(models, symbol) {
    ...
    if (stripped && stripped !== symbol) {
        if (models.has(stripped))
            return models.get(stripped);
        ...
        if (models.has(cap))
            return models.get(cap);
        if (cap.endsWith("s") && models.has(cap.slice(0, -1)))
            return models.get(cap.slice(0, -1));
    }
    return null;
}
// ─── Source ────────────────────────────────────────────────────────────────
export const prismaSource = {
    ...
    extract(filePath, symbol) {
        ...
        if (!parsed || parsed.models.size === 0)
            return null;
        if (symbol) {
            const direct = resolveModelName(parsed.models, symbol);
            if (direct)
                return capShapeSize(direct);
        }
...
// ─── Test hooks ────────────────────────────────────────────────────────────
export function __resetPrismaCacheForTesting() {
    rootToSchema.clear();
    schemaCache.clear();
}
