...
export const zodSource = {
    ...
    extract(filePath, symbol) {
        ...
        const firstName = env.schemas.keys().next().value;
        if (!firstName)
            return null;
...
function collectZodSchemas(ts, sf) {
...
function isZodExpression(ts, expr) {
...
function findValidatorTargetInFn(ts, fn) {
    let target = null;
    function visit(n) {
        if (target)
            return;
        if (ts.isCallExpression(n) &&
            ...
            ["parse", "safeParse", "parseAsync", "safeParseAsync"].includes(n.expression.name.text)) {
...
function walkZod(ts, expr, env, seen) {
    ...
    const mods = {};
    let head = expr;
    while (ts.isCallExpression(head) &&
    ...
    if (!ts.isCallExpression(head) ||
        ...
        if (ts.isIdentifier(head) && env.schemas.has(head.text)) {
            if (seen.has(head.text))
                return { $ref: head.text, _symbol: head.text };
    ...
    const fnName = head.expression.name.text;
    ...
    switch (fnName) {
        ...
        case "tuple": {
            ...
            if (arr && ts.isArrayLiteralExpression(arr)) {
                for (const el of arr.elements)
                    items.push(walkZod(ts, el, env, seen));
            }
        ...
        case "union": {
            ...
            if (arr && ts.isArrayLiteralExpression(arr)) {
                for (const el of arr.elements)
                    variants.push(walkZod(ts, el, env, seen));
            }
    ...
    if (mods.format && out.type === "string")
        out.format = mods.format;
    return applyMods(out, mods);
}
function walkZodObject(ts, arg, env, seen) {
    const properties = {};
    const required = [];
    ...
    for (const prop of arg.properties) {
        ...
        if (!isOptionalChain(ts, prop.initializer))
            required.push(key);
    }
    return { type: "object", properties, required };
}
function isOptionalChain(ts, expr) {
    let cur = expr;
    while (ts.isCallExpression(cur) &&
        ts.isPropertyAccessExpression(cur.expression) &&
        cur.expression.name &&
        ts.isIdentifier(cur.expression.name)) {
        if (cur.expression.name.text === "optional")
            return true;
        cur = cur.expression.expression;
    }
    return false;
}
function applyMods(shape, mods) {
