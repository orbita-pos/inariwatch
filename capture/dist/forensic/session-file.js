 ...
 * @internal
 */
export function __setSessionDirForTest(dir) {
    overrideDir = dir;
}
function sessionDir() {
    return overrideDir ?? DEFAULT_SESSION_DIR;
}
function sessionFilePath(pid) {
 ...
 */
function writeSessionFile(pid, ctx) {
...
function removeSessionFile(filePath) {
    try {
 ...
 */
export function installSessionFile(ctx) {
    if (!ctx.sessionId) {
 ...
 */
export function uninstallSessionFile() {
    if (current === null) {
        return;
    }
    if (current.active) {
 ...
 * @internal
 */
export function __sessionFileState() {
    return current;
}
