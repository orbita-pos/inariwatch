 ...
 * the capture ingest.
 */
export async function registerForensicHook(hook, options = {}) {
    if (mode !== null) {
...
export async function unregisterForensicHook() {
    if (mode === "fork")
        await fork.uninstall();
    else if (mode === "inspector")
        await fallback.uninstall();
    mode = null;
}
/** Exposed for tests and debugging. */
export function __mode() {
    return mode;
}
