...
function valueToSerialized(v) {
    return { type: "object", preview: v.repr, truncated: !!v.truncated };
}
function captureToForensicsField(c) {
    const locals = {};
    const closureChains = {};
    for (const f of c.frames) {
        if (f.locals.length > 0) {
            locals[String(f.index)] = Object.fromEntries(f.locals.map((l) => [l.name, valueToSerialized(l)]));
        }
        if (f.closure.length > 0) {
            closureChains[String(f.index)] = Object.fromEntries(f.closure.map((cv) => [cv.name, valueToSerialized(cv)]));
        }
    }
    return { locals, closureChains };
}
function matchScore(capture, event) {
...
export function forensicIntegration(config = {}) {
    ...
    return {
        ...
        setup() {
            ...
            registerForensicHook((capture) => {
                ...
                while (buffered.length > bufferMax)
                    buffered.shift();
            }, config).catch(() => {
                registered = false;
            });
...
export function __pushCaptureForTesting(c) {
    buffered.push(c);
    while (buffered.length > bufferMax)
        buffered.shift();
}
/** Test-only: clear buffer and reset registration state. */
export function __resetForensicIntegrationForTesting() {
    buffered = [];
    registered = false;
