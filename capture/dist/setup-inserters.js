...
function alreadyPresent(content) {
 ...
 */
export function insertViteConfig(content) {
 ...
 * `defineNuxtConfig({ ... })`.
 */
export function insertNuxtConfig(content) {
    if (alreadyPresent(content)) {
        return { content, status: "already-present" };
    }
    ...
    if (openBrace.test(content)) {
        return {
            content: content.replace(openBrace, `defineNuxtConfig({\n  modules: ["@inariwatch/capture/nuxt"],`),
 ...
 */
export function insertAstroConfig(content) {
    if (alreadyPresent(content)) {
        return { content, status: "already-present" };
    }
    const importLine = `import { inariwatchVite } from "@inariwatch/capture/vite"\n`;
 ...
 */
export function insertNextConfig(content) {
    if (alreadyPresent(content)) {
        return { content, status: "already-present" };
    }
    ...
    if (esmMatch) {
        ...
        const newContent = importLine + content.replace(/export default (\w+)/, "export default withInariWatch($1)");
        return { content: newContent, status: "inserted" };
    }
...
//# sourceMappingURL=setup-inserters.js.map