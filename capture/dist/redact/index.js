 ...
 */
import { DEFAULT_PATTERNS, IPV4_PATTERN, AWS_SECRET_PATTERN, } from "./patterns.js";
import { SENSITIVE_KEYS } from "./keys.js";
import { fnv1a32 } from "./hash.js";
import { resolveRedactConfig } from "./config.js";
// `RedactConfig` + `resolveRedactConfig` moved to `./config.js` so
// `client.ts` can static-import the tiny normalizer without pulling
// the full pattern/hash machinery. The re-exports below preserve the
// public API — anyone importing them from this module sees no change.
// See docs/decisions/0001-lazy-redact.md.
export { resolveRedactConfig };
...
export function redactPayload(payload, config = {}) {
    ...
    const patterns = [...DEFAULT_PATTERNS, ...opts.customPatterns];
    if (opts.redactIPs)
...
function walk(value, path, depth, opts, patterns, seen) {
    if (depth > opts.maxDepth)
        return value;
    if (value === null || value === undefined)
        return value;
    if (opts.allowlist.has(path))
        return value;
    const t = typeof value;
    if (t === "string") {
        return scrubString(value, opts, patterns);
    }
    if (t !== "object")
        return value;
...
function scrubString(input, opts, patterns) {
    if (input.length < 4)
        return input;
    let s = input;
    for (const p of patterns) {
        ...
        s = s.replace(p.regex, (match, ..._args) => {
            ...
            if (p.validate && !p.validate(match))
                return match;
            // Context check for AWS secret false-positive guard.
            if (p.label === "AWS_SECRET" && !opts.redactAwsSecrets)
                return match;
            if (p.label === "AWS_SECRET" && !AWS_CONTEXT_RE.test(input))
                return match;
