...
let pendingHttpProvenance = null;
...
export function tagValue(value, fromNodeId) {
    if (!fromNodeId)
        return;
    ...
    if (typeof value !== "object")
        return;
    provenance.set(value, fromNodeId);
}
...
export function getProvenance(value) {
    ...
    if (typeof value !== "object")
        return null;
    return provenance.get(value) ?? null;
}
/**
 ...
 */
export function findDataFromIds(args) {
    const list = Array.isArray(args) ? args : [args];
    const out = new Set();
    for (const arg of list) {
        if (!arg || typeof arg !== "object")
            continue;
        const direct = provenance.get(arg);
        if (direct)
            out.add(direct);
        for (const v of Object.values(arg)) {
            if (!v || typeof v !== "object")
                continue;
            const nested = provenance.get(v);
            if (nested)
                out.add(nested);
        }
    }
 ...
 */
export function markPendingHttpProvenance(fromNodeId) {
    if (!fromNodeId)
        return;
 ...
 */
export function installJsonParseTaint() {
    if (jsonParsePatched)
        return;
...
export function __resetDataFlowForTesting() {
...
export function __getPendingForTesting() {
