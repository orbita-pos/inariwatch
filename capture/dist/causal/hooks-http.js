 ...
 */
export async function installHttpHook(loader) {
    if (installedNode)
        return false;
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    dc.subscribe("undici:request:create", (payload) => {
        ...
        if (sessionId)
            injectUndiciHeader(req, "x-iw-session-id", sessionId);
        active.set(req, { id: handle.id, end: handle.end, start: Date.now(), url, method });
    });
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    dc.subscribe("undici:request:trailers", (payload) => {
        const req = payload?.request;
        if (!req || typeof req !== "object")
            return;
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    dc.subscribe("undici:request:error", (payload) => {
        const req = payload?.request;
        if (!req || typeof req !== "object")
            return;
        const slot = active.get(req);
 ...
 */
export function installBrowserHttpHook() {
    if (installedBrowser)
        return false;
    ...
    g.fetch = async function patchedFetch(input, init) {
        const method = (typeof input === "object" && "method" in input
            ? input.method
        ...
        if (!headers.has("x-iw-causal-id"))
            headers.set("x-iw-causal-id", handle.id);
        const sessionId = readSessionId();
        ...
        if (subgraphHeader) {
            const foreign = deserializeFromHeader(subgraphHeader);
            if (foreign)
                mergeSubgraph(foreign, safeOrigin(url), handle.id);
        }
...
// ─── Helpers ────────────────────────────────────────────────────────────────
function injectUndiciHeader(req, key, value) {
    try {
...
// ─── Test seams ─────────────────────────────────────────────────────────────
export function __resetHttpHookForTesting() {
    installedNode = false;
    installedBrowser = false;
}
export const __HTTP_PATCH_MARK_FOR_TESTING = PATCH_MARK;
export const __HTTP_HEADERS = {
    SESSION: SESSION_HEADER,
    CAUSAL: CAUSAL_HEADER,
    SUBGRAPH: SUBGRAPH_HEADER,
};
//# sourceMappingURL=hooks-http.js.map