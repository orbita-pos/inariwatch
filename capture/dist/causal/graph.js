...
function isFlagOn() {
    if (typeof process === "undefined" || !process.env)
        return false;
    const v = process.env.CAPTURE_CAUSAL_GRAPH ?? process.env.INARIWATCH_CAUSAL_GRAPH;
    return v === "1" || v === "true";
}
// ─── Lifecycle ─────────────────────────────────────────────────────────────
/**
 ...
 */
export async function initCausalGraph() {
    if (initialized)
        return;
    initialized = true;
...
function getOrCreateSlot() {
    ...
    if (!globalSlot)
        globalSlot = { buffer: newBuffer(), currentNodeId: null };
    return globalSlot;
}
 ...
 */
export function runWithRoot(fn) {
    if (!isFlagOn() || !als)
        return fn();
 ...
 */
export function recordOp(op, attrs) {
    if (!isFlagOn())
        return NOOP_HANDLE;
    const slot = getOrCreateSlot();
    const id = genId();
    const node = { id, op, ts: Date.now() };
    if (attrs)
        node.attrs = attrs;
    ...
    return {
        id,
            ...
            if (slot.currentNodeId === id)
                slot.currentNodeId = previousCurrent;
            if (slot.buffer.evictedIds.has(id))
                return;
            ...
            if (extras?.dataFrom?.length) {
                for (const fromId of extras.dataFrom) {
                    if (!fromId || slot.buffer.evictedIds.has(fromId))
                        continue;
 ...
 */
export function extractSubgraph(rootId, maxDepth = 5, maxNodes = MAX_NODES) {
    if (!isFlagOn())
        return undefined;
    const slot = getActiveSlot();
    if (!slot || slot.buffer.nodes.length === 0)
        return undefined;
    const start = rootId ??
        slot.currentNodeId ??
...
export function serializeForPayload(maxNodes = MAX_NODES) {
    if (!isFlagOn())
        return undefined;
    const slot = getActiveSlot();
    if (!slot || slot.buffer.nodes.length === 0)
 ...
 */
export function mergeSubgraph(foreign, prefix, parentId) {
    if (!isFlagOn())
    ...
    if (remaining <= 0)
        return { merged: 0, skipped: foreign.nodes.length };
    ...
    for (const fe of foreign.edges) {
        if (!accepted.has(fe.to) || !accepted.has(fe.from))
            continue;
        inboundCount.set(fe.to, (inboundCount.get(fe.to) ?? 0) + 1);
    }
    for (const fe of foreign.edges) {
        if (!accepted.has(fe.from) || !accepted.has(fe.to))
            continue;
        const from = idMap.get(fe.from);
        const to = idMap.get(fe.to);
        if (!from || !to)
            continue;
        const kind = fe.kind === "data" ? "data-flow" : fe.kind;
        buf.edges.push({ from, to, kind });
        if (kind === "causal")
            buf.parent.set(to, from);
    }
    if (parentId) {
        for (const fn of incomingNodes) {
 ...
 */
export function serializeForHeader(maxBytes = 8192) {
    if (!isFlagOn())
        return null;
...
export function deserializeFromHeader(header) {
    if (!header)
        return null;
    let json;
    try {
...
function pushNode(buf, node) {
    ...
    if (buf.nodes.length > MAX_NODES) {
        ...
        if (buf.edges.length > 0) {
            buf.edges = buf.edges.filter((e) => e.from !== evicted.id && e.to !== evicted.id);
        }
        // Cap evictedIds so it doesn't grow unbounded across long-lived
        // global-slot processes (workers, daemons).
        if (buf.evictedIds.size > MAX_NODES * 4)
            buf.evictedIds.clear();
        // lastSibling pointers may now reference an evicted id — it's a
        // best-effort; next recordOp under that parent will overwrite.
    }
}
function addEdge(buf, edge) {
    if (buf.evictedIds.has(edge.from) || buf.evictedIds.has(edge.to))
        return;
    buf.edges.push(edge);
}
function serializeBuffer(buf, maxNodes) {
    if (buf.nodes.length === 0)
        return undefined;
...
function toCausalNode(n) {
    ...
    if (n.attrs && n.attrs.error !== undefined) {
        labelExtras.push("err=1");
    }
    const label = labelExtras.length > 0 ? `${n.op} (${labelExtras.join(" ")})` : n.op;
    return {
        id: n.id,
...
export function __resetCausalGraphForTesting() {
    initialized = false;
...
export function __getBufferForTesting() {
    const slot = getActiveSlot();
    return slot ? slot.buffer : null;
}
export function __getCurrentIdForTesting() {
    return getActiveSlot()?.currentNodeId ?? null;
}
export function __isAlsActiveForTesting() {
    return als !== null;
}
/** Force the flag on for a single test block — restores on return. */
export function __withFlagOnForTesting(fn) {
    ...
    try {
        ...
        if (prev1 === undefined)
            delete env.CAPTURE_CAUSAL_GRAPH;
        else
            env.CAPTURE_CAUSAL_GRAPH = prev1;
        if (prev2 === undefined)
            delete env.INARIWATCH_CAUSAL_GRAPH;
        else
            env.INARIWATCH_CAUSAL_GRAPH = prev2;
    }
}
