 ...
 */
export async function installRedisHook(loader = DEFAULT_LOADER) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let mod;
    try {
        mod = await loader();
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function patchSendCommand(proto) {
    if (proto[PATCH_MARK])
        return false;
    ...
    if (typeof original !== "function")
        return false;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    proto.sendCommand = function patchedSendCommand(command) {
        ...
        if (promise && typeof promise.then === "function") {
            ...
            (value) => {
                ...
                if (handle.id && value && typeof value === "object")
                    tagValue(value, handle.id);
...
function extractCommandArgs(command) {
    if (!command || typeof command !== "object")
        return [];
    const obj = command;
...
export const __REDIS_PATCH_MARK_FOR_TESTING = PATCH_MARK;
//# sourceMappingURL=hooks-redis.js.map