 ...
 */
export async function installDrizzleHook(loaders = DEFAULT_LOADERS) {
    let patched = false;
    patched =
    ...
    patched =
        (await tryPatch(loaders.mysql, ["MySqlDatabase"], "drizzle.mysql")) ||
            patched;
    patched =
        (await tryPatch(loaders.sqlite, ["BaseSQLiteDatabase", "SQLiteDatabase"], "drizzle.sqlite")) || patched;
    return patched;
}
async function tryPatch(loader, classNames, opPrefix) {
    if (!loader)
        return false;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let mod;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function patchExecute(proto, opPrefix) {
    if (proto[PATCH_MARK])
        return false;
    ...
    if (typeof original !== "function")
        return false;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    proto.execute = function patchedExecute(query) {
...
function extractDrizzleSql(query) {
    if (!query)
        return "<unknown>";
...
// ─── Test helpers ──────────────────────────────────────────────────────────
export const __DRIZZLE_PATCH_MARK_FOR_TESTING = PATCH_MARK;
//# sourceMappingURL=hooks-drizzle.js.map