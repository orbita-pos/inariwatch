...
export function isProjectToken(value) {
...
export function parseDSN(dsn) {
    ...
    const authMode = isProjectToken(secretKey) ? "token" : "hmac";
 ...
 */
export function parseToken(token, projectId, hostOverride) {
...
export function createLocalTransport(_config) {
    return {
        send(event) {
...
export function createTransport(config, parsed) {
    const retryBuffer = [];
    function log(msg) {
        if (config.silent)
            return;
        if (config.debug)
            console.warn(`[@inariwatch/capture] ${msg}`);
    }
