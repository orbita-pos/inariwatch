 ...
 */
function isApproximatelyNonDecreasing(values, tolerance) {
    if (values.length < LEAK_WINDOW)
        return false;
 ...
 */
function hasMeaningfulEndGrowth(values, minPct, minAbs) {
...
export function installMemoryLeak(_config) {
    ...
    function sample() {
        setTimeout(() => {
            ...
            const domLeaking = isApproximatelyNonDecreasing(domValues, DOM_GROWTH_TOLERANCE) &&
                ((domValues[domValues.length - 1] ?? 0) - (domValues[0] ?? 0)) >= DOM_END_GROWTH_MIN_NODES;
            if (heapLeaking || domLeaking) {
                leakReported = true;
                const first = samples[0];
                const last = samples[samples.length - 1];
                captureLog(`suspected_memory_leak: ${heapLeaking ? "heap" : ""}${heapLeaking && domLeaking ? "+" : ""}${domLeaking ? "dom" : ""} growing across ${samples.length} navigations`, "warn", {
                    kind: "memory_leak",
