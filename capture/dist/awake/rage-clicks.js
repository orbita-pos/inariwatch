...
export function installRageClicks(config) {
    ...
    document.addEventListener("click", (e) => {
        ...
        if (recent.length >= rageCount) {
            const cluster = recent.filter(c => Math.hypot(c.x - x, c.y - y) <= rageRadius);
            if (cluster.length >= rageCount && ts - lastRageTs > 2000) {
                lastRageTs = ts;
                captureLog(`rage_click: ${cluster.length}x on ${elSelector(el)}`, "warn", {
                    kind: "rage_click",
        ...
        // flip it later.
        if (isInteractive(el)) {
            const entry = { el, ts, mutated: lastMutationTs >= ts };
            pending.push(entry);
            setTimeout(() => {
                ...
                if (!entry.mutated) {
                    captureLog(`dead_click: ${elSelector(el)}`, "warn", {
                        kind: "dead_click",
                        element: elSelector(el),
                        url: location.href,
                        pathname,
...
//# sourceMappingURL=rage-clicks.js.map