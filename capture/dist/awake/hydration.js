...
const WRAP_MARKER = "__inariwatchAwakeHydrationWrap";
let installed = false;
function detectHydrationError(args) {
    ...
    for (const { re, framework } of HYDRATION_PATTERNS) {
        if (!re.test(message))
            continue;
        captureLog(`hydration_mismatch: ${message.slice(0, 120)}`, "warn", {
...
export function installHydrationDetector() {
