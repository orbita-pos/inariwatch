...
export function installAwake(config = {}) {
    ...
    if (!skip.has("broken-resources"))
        installBrokenResources();
    if (!skip.has("loaf"))
        installLoAF(config);
    if (!skip.has("spa-routing"))
        installSpaRouting(config);
    if (!skip.has("web-vitals"))
        installWebVitals(config);
    if (!skip.has("resource-audit"))
        installResourceAudit(config);
    ...
    onIdle(() => {
        if (!skip.has("rage-clicks"))
            installRageClicks(config);
        if (!skip.has("memory-leak"))
            installMemoryLeak(config);
        if (!skip.has("storage-quota")) {
            void checkStorageQuota();
        }
        if (!skip.has("dom-size"))
            checkDomSize(config);
        if (!skip.has("hydration"))
            installHydrationDetector();
