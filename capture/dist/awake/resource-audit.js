...
const DEFAULT_OVERSIZED_BYTES = 500000;
...
const THIRD_PARTY_FLUSH_INTERVAL_MS = 10000;
...
function getThirdPartyLabel(hostname) {
    return THIRD_PARTY_LABELS[hostname] ?? hostname;
}
export function installResourceAudit(config) {
    ...
    const thirdPartyTotals = new Map();
    ...
    function shouldEmit(kind) {
        if (emittedCounts[kind] >= RESOURCE_EVENT_CAP_PER_KIND)
            return false;
        emittedCounts[kind]++;
        return true;
    }
    function flushThirdParty() {
        for (const [hostname, data] of thirdPartyTotals) {
            if (data.durationMs < 200)
                continue;
            captureLog(`third_party_impact: ${getThirdPartyLabel(hostname)} ${Math.round(data.durationMs)}ms`, data.durationMs > 1000 ? "warn" : "info", {
        ...
        new PerformanceObserver((list) => {
            for (const raw of list.getEntries()) {
                ...
                if (entry.initiatorType === "img" && entry.duration >= slowImageMs && shouldEmit("slow_image")) {
                    ...
                    captureLog(`slow_image: ${Math.round(entry.duration)}ms ${entry.name}`, levelForRating(rating), {
                        kind: "slow_image",
                        url: entry.name,
                ...
                if (entry.initiatorType === "img" && entry.transferSize > oversizedBytes && shouldEmit("oversized_image")) {
                    captureLog(`oversized_image: ${Math.round(entry.transferSize / 1024)}KB ${entry.name}`, "warn", {
                ...
                if (entry.renderBlockingStatus === "blocking" &&
                    ...
                    captureLog(`render_blocking: ${Math.round(entry.duration)}ms ${entry.name}`, entry.duration > 500 ? "error" : "warn", {
                ...
                if ((entry.initiatorType === "fetch" || entry.initiatorType === "xmlhttprequest") &&
                    ...
                    captureLog(`slow_fetch: ${Math.round(entry.duration)}ms ${entry.name}`, levelForRating(rating), {
                        kind: "slow_fetch",
                        url: entry.name,
                        durationMs: Math.round(entry.duration),
                        rating,
                ...
                try {
                    ...
                    if (entryHost && entryHost !== location.hostname) {
                        ...
                        if (entry.initiatorType === "script")
                            existing.scriptMs += entry.duration;
...
//# sourceMappingURL=resource-audit.js.map