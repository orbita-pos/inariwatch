...
const WARN_MS = 50;
const POOR_MS = 200;
function rateBlocking(ms) {
    return ms >= POOR_MS ? "poor" : "needs-improvement";
}
export function installLoAF(config) {
        ...
        try {
            new PerformanceObserver((list) => {
                for (const raw of list.getEntries()) {
                    ...
                    if (blockingMs < WARN_MS)
                        continue;
                    const rating = rateBlocking(blockingMs);
                    if (minRating === "poor" && rating !== "poor")
                        continue;
                    ...
                    captureLog(`loaf: ${Math.round(blockingMs)}ms blocking${thrashingScript ? " (layout thrash)" : ""}`, rating === "poor" ? "error" : "warn", {
        ...
        try {
            new PerformanceObserver((list) => {
                for (const entry of list.getEntries()) {
                    if (entry.duration < WARN_MS)
                        continue;
                    const rating = rateBlocking(entry.duration);
                    if (minRating === "poor" && rating !== "poor")
                        continue;
                    captureLog(`long_task: ${Math.round(entry.duration)}ms`, rating === "poor" ? "error" : "warn", {
                        kind: "long_task",
