 ...
 */
export async function persistTombstone(tombstone) {
    ...
    try {
        ...
        const [fs, path, os] = await Promise.all([
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            import(/* webpackIgnore: true */ fsPkg),
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            import(/* webpackIgnore: true */ pathPkg),
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            import(/* webpackIgnore: true */ osPkg),
        ]);
        const dir = process.env.INARIWATCH_TOMBSTONE_DIR ?? path.join(os.homedir(), ".inariwatch");
 ...
 */
export function extractTombstone(json) {
    if (!json || typeof json !== "object")
        return null;
    const obj = json;
    if (obj.mode !== "zero-retention")
        return null;
