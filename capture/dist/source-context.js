 ...
 */
export function getSourceContext(stack) {
    const frames = parseStackForEvidence(stack);
    const out = [];
    for (let idx = 0; idx < frames.length; idx++) {
        const f = frames[idx];
        if (!f.file || f.file === "<unknown>" || f.line <= 0)
            continue;
        const absPath = resolveFramePath(f.file);
        if (!absPath)
            continue;
        if (absPath.includes(`${sep}node_modules${sep}`))
            continue;
 ...
 */
function resolveFramePath(rawFile) {
    let p = rawFile;
...
function readSourceSlice(absPath, lineNum) {
...
// ───────────────────── Git blame ───────────────────────────────────────────
function getBlame(absPath, line) {
...
function findRepoRoot(start) {
    ...
    while (dir) {
        ...
        if (cached !== undefined) {
            ...
            for (const s of seen)
                repoRootCache.set(s, cached);
            return cached;
        }
        seen.push(dir);
        if (existsSync(`${dir}${sep}.git`)) {
            for (const s of seen)
                repoRootCache.set(s, dir);
            return dir;
        }
...
export function __resetSourceContextCachesForTesting() {
    fileCache.clear();
    blameCache.clear();
    repoHeadCache.clear();
    repoRootCache.clear();
